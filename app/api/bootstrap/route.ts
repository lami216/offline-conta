...
export async function GET(request: Request) {const licenseDenied=await requireValidLicense();if(licenseDenied)return licenseDenied;
  ...
  try {
    ...
    const customerPartyIds=new Set(activeParties.filter(party=>resolvePartyType(party)==="customer").map(party=>String(party.id)));
    const supplierPartyIds=new Set(activeParties.filter(party=>resolvePartyType(party)==="supplier").map(party=>String(party.id)));
    ...
    const exposedParties=(activeParties as Array<Record<string,unknown>>).map(party=>hasCapability(principal,resolvePartyType(party)==="supplier"?"suppliers.view":"customers.view")?party:{id:party.id,name:party.name,phone:party.phone,partyType:party.partyType,receivable:0,payable:0,net:0});
    ...
    const visiblePartyIds=new Set(activeParties.filter(party=>(resolvePartyType(party)==="customer"&&hasCapability(principal,"customers.view"))||(resolvePartyType(party)==="supplier"&&hasCapability(principal,"suppliers.view"))).map(party=>String(party.id)));
    ...
    return Response.json({ branding, principal:{principalType:principal.principalType,name:principal.name,permissions:principal.permissions}, parties:exposedParties, warehouses:clean(warehouses), products:exposedProducts, categories:cleanCategories, documents:allowedDocuments, movements:hasCapability(principal,"warehouses.inventory.view")?cleanMovements:[], financialMovements:bankAccess?clean(effectiveFinancialMovements):[], partyFinancialSummaries, paymentAccounts:selectorAccounts, accountTransfers:hasCapability(principal,"banks.transfer")?clean(effectiveTransfers):[], nextProductCode, nextDocumentSequences:{sale:nextSale,purchase:nextPurchase,expense:nextExpense} });
