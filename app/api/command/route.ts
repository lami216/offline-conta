...
async function financialMovement(db: Db, session: ClientSession, document: Record<string, unknown>, direction: "in" | "out", amount: number, type: string) {
  ...
  const negativeBalanceAllowed = account.allowNegativeBalance === true;
...
export async function execute(db: Db, session: ClientSession, body: Input) {
  ...
  if (["sale.update", "purchase.update"].includes(type)) {
    ...
    for (const line of input) {
      ...
      if (isSale) {
        ...
        if (cost != null && unitPrice < Number(cost)) throw new CommandError(`لا يمكن البيع تحت سعر الشراء. سعر الشراء التاريخي: ${cost} MRU`);
  ...
  if(type==="recurring-expense.update"){
    const recurringId=text(body.recurringId),title=text(body.title),amount=positive(body.amount,"المبلغ"),frequency=text(body.frequency),startsOn=text(body.startsOn);
    if(!title||!["daily","monthly"].includes(frequency)||!/^\d{4}-\d{2}-\d{2}$/.test(startsOn))throw new CommandError("بيانات المصروف المتكرر غير صالحة");
    const result=await db.collection("recurringExpenses").updateOne({id:recurringId,active:true},{$set:{title,amount,frequency,startsOn,updatedAt:new Date()}},{session});
    if(!result.matchedCount)throw new CommandError("المصروف المتكرر غير موجود",404);return recurringId;
  }
  if (type === "expense.materialize") { const recurringId = text(body.recurringId), dueDate = text(body.dueDate), method = text(body.paymentMethod), recurring = await db.collection("recurringExpenses").findOne({ id: recurringId, active: true }, { session }); if (!recurring) throw new CommandError("المصروف المتكرر غير موجود", 404); await paymentAccount(db, session, method); const occurrenceKey = recurring.frequency === "monthly" ? dueDate.slice(0, 7) : dueDate; const existing = await db.collection("documents").findOne({ recurringId, occurrenceKey }, { session }); if (existing) throw new CommandError("تم تسجيل دفع هذا الاستحقاق مسبقًا", 409); const doc = { ...await numberedDocument(db, session, "expense", "EXP"), occurredAt: new Date(`${dueDate}T12:00:00Z`).toISOString(), recurringId, dueDate, occurrenceKey, partyId: null, partyName: null, warehouseId: null, warehouseName: null, destinationWarehouseId: null, destinationWarehouseName: null, parentDocumentId: null, paymentMethod: method, title: recurring.title, total: recurring.amount, dueTotal: 0, paidTotal: recurring.amount, lines: [{ id: id("line"), productId: null, description: recurring.title, quantity: 1, unitPrice: recurring.amount, lineTotal: recurring.amount }] }; await financialMovement(db, session, doc, "out", Number(recurring.amount), "expense"); await db.collection("documents").insertOne(doc, { session }); return doc.id; }
  if (type === "payment-account.update") { const accountId=text(body.id),name=text(body.name);if(!name||typeof body.allowNegativeBalance!=="boolean")throw new CommandError("بيانات وسيلة الدفع غير صالحة");const color=text(body.color),account=await paymentAccount(db,session,accountId,false),allowNegativeBalance=body.allowNegativeBalance===true;if(Number(account.balance??0)<0&&!allowNegativeBalance)throw new CommandError("لا يمكن إيقاف السماح بالرصيد السالب والحساب رصيده الحالي سالب. سوِّ الرصيد أولًا.",409);await db.collection("paymentAccounts").updateOne({id:accountId},{$set:{name,...(color&&/^#[0-9a-f]{6}$/i.test(color)?{color}:{}),isActive:body.isActive!==false,allowNegativeBalance,updatedAt:new Date()}},{session});return accountId; }
  if (type === "payment-account.create") { const name = text(body.name), openingBalance = positive(body.openingBalance ?? 0, "رصيد البداية", true); if (!name || (body.allowNegativeBalance !== undefined && typeof body.allowNegativeBalance !== "boolean")) throw new CommandError("اسم البنك أو وسيلة الدفع مطلوب"); const account = { id: id("account"), code: id("custom"), name, color: "#1677c8", icon: "wallet", isActive: true, allowNegativeBalance: body.allowNegativeBalance === true, balance: 0, createdAt: new Date() }; await db.collection("paymentAccounts").insertOne(account, { session }); if (openingBalance > 0) { const doc = { ...baseDocument("opening-balance", "OPEN"), paymentMethod: account.id, partyId: null, partyName: null, note: "رصيد بداية" }; await financialMovement(db, session, doc, "in", openingBalance, "opening-balance"); } return account.id; }
  if(type==="payment-account.delete"){const account=await paymentAccount(db,session,body.accountId,false);if(account.code==="cash")throw new CommandError("لا يمكن حذف وسيلة الدفع النقدية الأساسية",409);if(Number(account.balance??0)!==0)throw new CommandError("لا يمكن حذف أو أرشفة وسيلة الدفع ورصيدها غير صفري. صفّر أو سوِّ الرصيد أولًا.",409);const key=[account.id,account.code], [movement,document,transfer,recurring]=await Promise.all([db.collection("financialMovements").findOne({paymentMethod:{$in:key}},{session}),db.collection("documents").findOne({paymentMethod:{$in:key}},{session}),db.collection("accountTransfers").findOne({$or:[{fromAccountId:{$in:key}},{toAccountId:{$in:key}}]},{session}),db.collection("recurringExpenses").findOne({paymentMethod:{$in:key}},{session})]);if(movement||document||transfer||recurring){await db.collection("paymentAccounts").updateOne({id:account.id},{$set:{isActive:false,isArchived:true,archivedAt:new Date(),updatedAt:new Date()}},{session});return {id:String(account.id),disposition:"archived"}}await db.collection("paymentAccounts").deleteOne({id:account.id},{session});return {id:String(account.id),disposition:"deleted"}}
  ...
  if(type==="account-balance-correction.post"){const account=await paymentAccount(db,session,body.accountId,false),before=Number(account.balance??0),after=num(body.newBalance),reason=text(body.reason);if(!Number.isFinite(after)||(after<0&&account.allowNegativeBalance!==true))throw new CommandError("الرصيد الصحيح غير صالح");if(!reason)throw new CommandError("سبب التصحيح مطلوب");if(after===before)throw new CommandError("الرصيد الجديد يطابق الرصيد الحالي.");const delta=after-before,occurredAt=new Date().toISOString(),movementId=id("fin"),reference=`COR-${Date.now()}`;const updated=await db.collection("paymentAccounts").updateOne({id:account.id,balance:before},{$set:{balance:after,updatedAt:new Date()}},{session});if(!updated.matchedCount)throw new CommandError("تغير الرصيد أثناء العملية، أعد المحاولة",409);await db.collection("financialMovements").insertOne({id:movementId,paymentMethod:account.id,paymentCode:account.code,direction:delta>0?"in":"out",amount:Math.abs(delta),delta,balanceBefore:before,balanceAfter:after,reason,note:reason,type:"balance-correction",occurredAt,documentId:movementId,documentNumber:reference,partyId:null,partyName:null,transferId:null},{session});return movementId}
