...
export async function POST(request: Request) {const licenseDenied=await requireValidLicense();if(licenseDenied)return licenseDenied;
  ...
  try {
    ...
    if(type==="legacy-party-entry.void"){
      ...
      if(party){
        ...
        capability=document?.kind==="payment"?(supplier?"suppliers.pay.delete":"customers.collect.delete"):(supplier?"suppliers.edit":"customers.edit");
      }
    }
