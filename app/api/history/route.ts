...
/** Paginated history applies the same per-document access rules as bootstrap. */
export async function GET(request:Request){
 ...
 if(resource==="documents"&&!hasCapability(principal,"records.view")){
   const kinds=[...(hasCapability(principal,"pos.view")?["sale"]:[]),...(hasCapability(principal,"purchases.view")?["purchase"]:[]),...(hasCapability(principal,"expenses.view")?["expense"]:[])];
   if(kind&&!kinds.includes(kind))return Response.json({error:"غير مصرح"},{status:403});
   query.kind=kind??{$in:kinds};
 }else if(kind)query[resource==="documents"?"kind":"type"]=kind;
 if(from||to){
