...
export async function GET(request: Request) { const denied = await requireCapability(request, "settings.backup.manage"); if (denied) return denied; const db = await getDatabase(); const values = await Promise.all(BACKUP_COLLECTIONS.map(async name => [name, await db.collection(name).countDocuments()])); return Response.json({ counts: Object.fromEntries(values) }, { headers: { "cache-control": "no-store" } }); }
