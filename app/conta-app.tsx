...
function ContaAppContent() {
  ...
  const openDocumentSource = async (document: DocumentRecord) => {
    ...
    if(document.kind==="payment"&&document.partyId){const party=data.parties.find(item=>item.id===document.partyId);if(!party)return;const customer=resolvePartyType(party)==="customer",edit=Boolean(document.partyCashDirection)&&document.status==="posted"&&!party.isArchived&&can(customer?"customers.collect.edit":"suppliers.pay.edit");if(!await navigate(customer?"customers":"suppliers",{replaceEditor:edit}))return;setPartyDetail(party);if(edit)setPartyPaymentEditRequest(document.id);return}
  ...
  const canVoidLegacyPartyDocument=(document:DocumentRecord)=>{
    if(document.status!=="posted"||!((document.kind==="payment"&&!document.partyCashDirection)||document.kind==="settlement"||document.kind==="offset"))return false;
    const party=document.partyId?data.parties.find(item=>item.id===document.partyId):null;
    if(!party)return data.principal.principalType!=="user";
    const supplier=resolvePartyType(party)==="supplier";
    return canUseCapability(data.principal,document.kind==="payment"?(supplier?"suppliers.pay.delete":"customers.collect.delete"):(supplier?"suppliers.edit":"customers.edit"));
  };
  const voidLegacyPartyDocument=async(document:DocumentRecord)=>{
    if(!canVoidLegacyPartyDocument(document))return;
    if(!await confirmAction({message:tr("هل تريد حذف هذه الحركة القديمة؟ سيتم عكس أثرها المحاسبي إن كان ما زال قائمًا، مع الاحتفاظ بسجل التدقيق."),confirmLabel:tr("حذف"),tone:"danger"}))return;
    await run({type:"legacy-party-entry.void",documentId:document.id},tr("تم إلغاء الحركة القديمة"),closeDoc);
  };
        ...
        <div className="content">
          ...
          {loading ? (
            ...
            <>
              ...
              {view === "products" && <Products data={data} run={run} sourceRequest={productSourceRequest} clearSourceRequest={()=>setProductSourceRequest(null)} openProductMovements={id=>void openProductMovementHistory(id)} />}{" "}
...
function Banks({ data, run, openDoc, openSource, tab, sourceRequest, clearSourceRequest, registerEditorGuard, prepareEditorReplacement }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; openSource:(target:SummarySourceTarget)=>void; tab:BankTab; sourceRequest:BankSourceRequest|null; clearSourceRequest:()=>void; registerEditorGuard:RegisterEditorGuard; prepareEditorReplacement:()=>Promise<boolean> }) {
  ...
  const removeLegacyBalanceCorrection=async(movement:BootstrapData["financialMovements"][number])=>{
    if(!canBalanceDelete||financialMovementKind(movement.type)!=="balance-correction")return;
    if(!await confirmAction({message:tr("هل تريد حذف تصحيح الرصيد القديم؟ سيتم عكس أثره على الحساب مع الاحتفاظ بسجل التدقيق."),confirmLabel:tr("حذف"),tone:"danger"}))return;
    await run({type:"legacy-account-balance-correction.void",movementId:movement.id},tr("تم إلغاء تصحيح الرصيد القديم"));
  };
  ...
  {tab==="movements"&&<div className="bank-tab-content bank-tab-movements"><div className="bank-filter-stack"><div className="bank-filters"><div className="bank-scope-controls"><CompactDateRange from={movementScope.draftFrom} to={movementScope.draftTo} allTime={movementScope.period===null&&!accountFilter&&!typeFilter} onApply={movementScope.commit} onAllTime={resetMovementFilters} onFromChange={movementScope.setDraftFrom} onToChange={movementScope.setDraftTo}/></div><PaymentAccountSelect accounts={data.paymentAccounts} value={accountFilter} onChange={setAccountFilter} placeholder={tr("كل الحسابات")}/><select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="">{tr("كل الأنواع")}</option>{Object.entries(movementLabels).filter(([value])=>!["opening-balance","opening-balance-correction"].includes(value)).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>{noteClauses&&<small className="bank-scope-note">{tr("تنبيه: العرض الحالي حسب")} {noteClauses}</small>}</div><div className="erp-table-wrap ledger-list"><table className="erp-table"><thead><tr><SortableTableHeader column="date" label={tr("التاريخ")} sort={movementSort} toggle={toggleMovementSort}/><SortableTableHeader column="type" label={tr("النوع")} sort={movementSort} toggle={toggleMovementSort}/><SortableTableHeader column="account" label={tr("الحساب")} sort={movementSort} toggle={toggleMovementSort}/><SortableTableHeader column="amount" label={tr("الحركة")} sort={movementSort} toggle={toggleMovementSort}/><SortableTableHeader column="document" label={tr("المستند")} sort={movementSort} toggle={toggleMovementSort}/><th>{tr("إجراءات")}</th></tr></thead><tbody>{sortedMovements.map(m=><tr key={m.id} onClick={()=>inspectMovement(m)}><td>{formatDateTime(m.occurredAt)}</td><td>{movementLabels[financialMovementKind(m.type)]??financialMovementKind(m.type)}</td><td>{name(m.paymentMethod)}</td><td className={`num-cell ${m.direction==="in"?"bank-amount-positive":"bank-amount-negative"}`}><MoneyValue value={m.amount} tone={m.direction==="in"?"positive":"negative"}/></td><td dir="ltr">{m.documentNumber}</td><td className="action-cell">{financialMovementKind(m.type)==="balance-correction"?<LifecycleActions onVoid={canBalanceDelete?()=>void removeLegacyBalanceCorrection(m):undefined}/>:"—"}</td></tr>)}</tbody></table></div></div>}
  ...
  return <form className="panel product-form" onSubmit={async event=>{event.preventDefault();const sensitive=Boolean(product&&(name.trim()!==product.name||(cost===""?null:val(cost))!==product.pieceCost||openingDirty));const confirmed=sensitive?await confirmAction({message:tr("product.sensitiveChangeConfirm",{name:product?.name??name})}):true;if(!confirmed)return;
    ...
    <div className="product-form-halves">
      ...
      <FramedSection title={tr("الأسعار والمخزون")} className="product-form-group">
        ...
        {product&&openingState&&<small>{tr("opening.consumed")}: <b>{number(openingState.consumed)}</b> · {tr("opening.remaining")}: <b>{number(openingState.remaining)}</b>{desiredOpening>=openingState.consumed&&desiredOpening!==openingState.total?<> · {tr("opening.after")}: <b>{number(desiredOpening-openingState.consumed)}</b></>:null}{openingState.consumed>0&&<button type="button" className="link opening-movements-link" onClick={()=>{close();openProductMovements(product.id)}}>{tr("عرض حركات المنتج")}</button>}</small>}
...
function DocumentDetail({document,data,close,onEdit,onVoid,onSource,openLinked}:{document:DocumentRecord;data:BootstrapData;close:()=>void;onEdit?: () => void;onVoid?: () => void;onSource?: () => void;openLinked?: (id:string) => void}){
