...
function Parties({ partyType, data, run, openParty, canEdit, canDelete }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void; canEdit:boolean; canDelete:boolean }) {
  ...
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    <FramedSection title={`إدارة ${plural}`} className="parties-controls"><CompactSearch value={q} onChange={setQ} placeholder={tr("بحث بالاسم أو الهاتف")}/>{archived.length>0&&<button type="button" className="soft" onClick={()=>setShowArchived(true)}>{tr("المؤرشفون")} ({number(archived.length)})</button>}{canCreate&&<form className="parties-create" onSubmit={async e=>{e.preventDefault();await run({type:"party.create",partyType,name,phone},`تمت إضافة ${noun}`);setName("");setPhone("")}}><input required value={name} onChange={e=>setName(e.target.value)} placeholder={`اسم ${noun}`}/><input dir="ltr" value={phone} onChange={e=>setPhone(e.target.value)} placeholder={tr("رقم الهاتف")}/><button className="primary"><Plus/>  {tr("إضافة")} {noun}</button></form>}</FramedSection>
