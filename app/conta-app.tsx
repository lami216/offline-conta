...
function Expenses({ data, run, openDoc, registerEditorGuard, canEdit }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; registerEditorGuard: RegisterEditorGuard; canEdit: boolean }) {
  ...
  return <section className="expense-workspace workspace-page"><div className="expense-grid">
    <FramedSection title={editingExpenseId ? "تعديل المصروف" : editingRecurringId ? "تعديل المصروف المتكرر" : "مصروف جديد"} className="expense-form"><form className="expense-form-body" onSubmit={submit}><div className="expense-fields">
      ...
      <div className="expense-edit-actions"><button className="primary expense-save" disabled={!title||!amount||(!editingRecurringId&&frequency==="once"&&!paymentMethod)}>{editingExpenseId||editingRecurringId?"حفظ التعديل":frequency==="once"?"حفظ الفاتورة":"حفظ التذكير"}</button>{(editingExpenseId||editingRecurringId)&&<button type="button" className="soft expense-cancel" onClick={cancelEdit}>إلغاء التعديل</button>}</div>
    </div></form></FramedSection>
    <FramedSection title={`المصاريف المستحقة · ${number(data.recurringExpenses.length)}`} className="expense-recurring"><div className="erp-table-wrap expense-scroll"><table className="erp-table" aria-label="المصاريف المستحقة"><thead><tr><th>المصروف</th><th>الاستحقاق</th><th>المبلغ</th><th>الحالة / الحساب</th><th className="expense-action-column">إجراء</th></tr></thead><tbody>{data.recurringExpenses.map(r=><tr key={r.id}><td className="name-cell">{r.title}</td><td>{formatDate(r.currentDueDate)}</td><td className="num-cell">{money(r.amount)}</td><td>{r.currentPaymentMethodId?<button className="link" onClick={()=>{const doc=data.documents.find(d=>d.id===r.currentDocumentId);if(doc)loadExpense(doc)}}>{`مدفوع · ${accountName(r.currentPaymentMethodId)}`}</button>:r.frequency==="daily"?"يومي · غير مدفوع":"شهري · غير مدفوع"}</td><td className="action-cell expense-action-column"><div className="expense-row-actions">{!r.currentPaymentMethodId&&<button className="soft expense-pay" onClick={()=>{setRecurringPaymentMethod("");setPaying(r.id)}}>تسجيل الدفع</button>}{canEdit&&<button className="soft" onClick={()=>loadRecurring(r)}>تعديل</button>}</div></td></tr>)}{!data.recurringExpenses.length&&<tr><td colSpan={5}>لا توجد مصاريف متكررة</td></tr>}</tbody></table></div></FramedSection>
...
function Warehouses({ data, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  return <section className="warehouse-workspace"><FramedSection title="جرد المخازن" className="inventory-panel">
    <div className="inventory-overview inventory-toolbar"><SearchableSelect value={wh} onChange={chooseWarehouse} placeholder="اختر المخزن" searchPlaceholder="ابحث عن مخزن" floating options={availableWarehouses.map(w => ({ value: w.id, label: w.name }))}/><CompactDateRange from={draftFrom} to={draftTo} allTime={hasInventoryView&&committedPeriod===null} onApply={commitPeriod} onAllTime={showAllInventory} onFromChange={setDraftFrom} onToChange={setDraftTo}/><button className="soft inventory-print" disabled={!hasInventoryView} onClick={() => window.print()}><Printer /> طباعة الجرد</button><div className="inventory-overview-metrics"><span><small>عدد المنتجات</small><b>{hasInventoryView?number(inventoryProducts.length):"—"}</b></span><span><small>إجمالي الكمية</small><b>{hasInventoryView?number(totalPieces):"—"}</b></span><span><small>قيمة المخزون</small><b>{hasInventoryView?number(inventoryValue):"—"}</b></span></div></div>
