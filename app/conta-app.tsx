...
function Banks({ data, run, openDoc, openSource, tab, sourceRequest, clearSourceRequest, registerEditorGuard, prepareEditorReplacement }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; openSource:(target:SummarySourceTarget)=>void; tab:BankTab; sourceRequest:BankSourceRequest|null; clearSourceRequest:()=>void; registerEditorGuard:RegisterEditorGuard; prepareEditorReplacement:()=>Promise<boolean> }) {
  ...
  const active=activePaymentAccounts(data.paymentAccounts),visibleAccounts=data.paymentAccounts.filter(account=>account.isArchived!==true),archived=data.paymentAccounts.filter(account=>account.isArchived===true),name=useCallback((id:string)=>data.paymentAccounts.find(account=>account.id===id||account.code===id)?.name??id,[data.paymentAccounts]);
  ...
  const movementSummaryDetail=(direction:"in"|"out"):SummaryBreakdown=>{const source=direction==="in"?accountBreakdown.income:accountBreakdown.expenses;return{title:direction==="in"?tr("إجمالي المداخيل"):tr("إجمالي المصاريف"),total:direction==="in"?accountSummary.income:accountSummary.expenses,rows:source.map(row=>({id:`${direction}-${row.kind}`,label:summaryMovementLabels[row.kind]??row.kind,value:row.value,source:movementSummarySource(row.kind)}))}};
  const accountBalanceDetail:SummaryBreakdown={title:tr("إجمالي الأرصدة الحالية"),total:accountSummary.currentBalance,rows:accountBreakdown.accounts.map(account=>({id:`account-${account.id}`,label:account.name,meta:account.isActive?tr("نشط"):tr("متوقف"),value:account.value,source:{kind:"view",view:"banks",bankTab:"accounts"}}))};
  const debtDetail=(side:"owedToUs"|"weOwe"):SummaryBreakdown=>({title:side==="owedToUs"?tr("إجمالي المستحق لنا"):tr("إجمالي المستحق علينا"),total:accountSummary[side],rows:accountBreakdown.parties.map(party=>({id:`${side}-${party.id}`,label:party.name,meta:`${party.partyType==="customer"?tr("عميل"):tr("مورد")}${party.isArchived?` · ${tr("مؤرشف")}`:""}`,value:party[side],source:{kind:"party",partyId:party.id}}))});
