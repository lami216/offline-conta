...
import { isOpeningStockCorrectionDocument, isOpeningStockDocument, optionalFiniteNumber, stockMovementMatchesFilter } from "./stock-movement";
...
export function periodMovementQuantity(movements:BootstrapData["movements"],productId:string,warehouseIds:string|string[],kind:"purchase"|"sale",from:string,to:string){const ids=new Set(Array.isArray(warehouseIds)?warehouseIds:[warehouseIds]);return movements.filter(movement=>movement.productId===productId&&ids.has(movement.warehouseId)&&stockMovementMatchesFilter(movement.type,kind)&&(!from||movement.occurredAt.slice(0,10)>=from)&&(!to||movement.occurredAt.slice(0,10)<=to)).reduce((sum,movement)=>sum+(kind==="sale"?-Number(movement.quantityDelta):Number(movement.quantityDelta)),0)}
...
function Records({ openDoc }: { openDoc: (id: string) => void }) {
  const today=localBusinessDay(),[kind,setKind]=useState("sale"),[q,setQ]=useState(""),[from,setFrom]=useState(today),[to,setTo]=useState(today),[allTime,setAllTime]=useState(false),[page,setPage]=useState(1),[totalPages,setTotalPages]=useState(1),[total,setTotal]=useState(0),[docs,setDocs]=useState<DocumentRecord[]>([]),[busy,setBusy]=useState(false),[failure,setFailure]=useState("");
  useEffect(()=>{
    const controller=new AbortController(),timer=window.setTimeout(()=>{void(async()=>{
      const params=new URLSearchParams({resource:"documents",page:String(page),pageSize:"100"});
      ...
      try{const result=await readApiResponse(await fetch("/api/history?"+params.toString(),{signal:controller.signal})) as {rows:DocumentRecord[];total:number;totalPages:number};if(!controller.signal.aborted){setDocs(result.rows);setTotal(result.total);setTotalPages(Math.max(1,result.totalPages));if(page>Math.max(1,result.totalPages))setPage(Math.max(1,result.totalPages))}}
  ...
  return <section className="records-workspace">
    <FramedSection title={tr("بحث السجلات")} className="records-filters"><div className="filters">
      <CompactSearch value={q} onChange={value=>{setQ(value);setPage(1)}} placeholder={tr("رقم المستند أو الطرف")} />
      <select className="records-kind-filter" value={kind} onChange={e=>{setKind(e.target.value);setPage(1)}}><option value="">{tr("كل المعاملات")}</option>{Object.entries(visibleDocumentKindLabels).map(([k,v])=><option key={k} value={k}>{tr(v)}</option>)}</select>
      <CompactDateRange from={from} to={to} allTime={allTime} onAllTime={()=>{setFrom("");setTo("");setQ("");setKind("");setAllTime(true);setPage(1)}} onFromChange={value=>{setFrom(value);setAllTime(false);setPage(1)}} onToChange={value=>{setTo(value);setAllTime(false);setPage(1)}} />
    </div></FramedSection>
    {failure&&<div className="error">{failure}</div>}
    {busy?<div className="report-loading">{tr("جاري تحميل السجلات…")}</div>:<><Recent title={tr("كل السجلات القابلة للتتبع")} docs={docs} openDoc={openDoc}/><div className="report-pagination"><button type="button" className="soft" disabled={page<=1} onClick={()=>setPage(current=>Math.max(1,current-1))}>{tr("السابق")}</button><span>{tr("الصفحة")} {number(page)} / {number(totalPages)} · {number(total)} {tr("سجل")}</span><button type="button" className="soft" disabled={page>=totalPages} onClick={()=>setPage(current=>Math.min(totalPages,current+1))}>{tr("التالي")}</button></div></>}
  </section>;
}
