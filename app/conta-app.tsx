...
function Expenses({ data, run, openDoc, editRequest, clearEditRequest, registerEditorGuard, canEdit, canDelete }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; editRequest:string|null; clearEditRequest:()=>void; registerEditorGuard: RegisterEditorGuard; canEdit: boolean; canDelete:boolean }) {
  ...
  return <section className="expense-workspace workspace-page"><div className="expense-grid">
    ...
    <FramedSection title={tr("سجل المصاريف")} className="expense-history"><div className="expense-history-filters"><CompactSearch value={historyQuery} onChange={q=>applyExpenseFilters(expenseSearchMode(filters(),q))} placeholder={tr("بحث بالعنوان أو رقم المستند")}/><CompactDateRange from={historyFrom} to={historyTo} allTime={historyAllTime&&!historyQuery.trim()} onAllTime={()=>applyExpenseFilters(expenseAllTimeMode())} onFromChange={v=>applyExpenseFilters(expenseDateMode(filters(),"from",v))} onToChange={v=>applyExpenseFilters(expenseDateMode(filters(),"to",v))}/></div><div className="erp-table-wrap expense-scroll"><table className="erp-table" aria-label={tr("سجل المصاريف")}><thead><tr><SortableTableHeader column="number" label={tr("رقم المستند")} sort={expenseSort} toggle={toggleExpenseSort}/><SortableTableHeader column="date" label={tr("التاريخ")} sort={expenseSort} toggle={toggleExpenseSort}/><SortableTableHeader column="title" label={tr("العنوان")} sort={expenseSort} toggle={toggleExpenseSort}/><SortableTableHeader column="total" label={tr("المبلغ")} sort={expenseSort} toggle={toggleExpenseSort}/><SortableTableHeader column="payment" label={tr("وسيلة الدفع")} sort={expenseSort} toggle={toggleExpenseSort}/></tr></thead><tbody>{sortedExpenses.map(document=><tr key={document.id} onClick={()=>loadExpense(document)}><td dir="ltr">{displayDocumentNumber(document)}</td><td>{formatDate(document.occurredAt)}</td><td className="name-cell">{document.title??tr("مصروف")}</td><td className="num-cell">{money(document.total)}</td><td>{accountName(document.paymentMethod)}</td></tr>)}{!expenseDocs.length&&<tr><td colSpan={5}>{tr("لا توجد فواتير مطابقة")}</td></tr>}</tbody></table></div></FramedSection>
  </div></section>;
}
...
function Banks({ data, run, openDoc, openSource, tab, sourceRequest, clearSourceRequest }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; openSource:(target:SummarySourceTarget)=>void; tab:BankTab; sourceRequest:BankSourceRequest|null; clearSourceRequest:()=>void }) {
  ...
  const movementSummarySource=(kind:string):SummarySourceTarget|undefined=>{
    if(kind==="sale")return{kind:"report",reportType:"sales"};
    if(kind==="purchase")return{kind:"report",reportType:"purchases"};
    if(kind==="expense")return{kind:"report",reportType:"expenses"};
...
function PartyAggregateMetrics({partyType,metrics,net}:{partyType:"customer"|"supplier";metrics:ReturnType<typeof partyAggregateMetrics>;net:number}) {
  const dues:PartyMetricStripItem={labelKey:net>0?"إجمالي المستحقات لنا":net<0?"إجمالي المستحقات علينا":"إجمالي المستحقات",value:Math.abs(net),tone:net>0?"positive":net<0?"negative":"neutral"};
...
function Reports({ data, openDoc, openSource, type }: { data: BootstrapData; openDoc: (id: string) => void; openSource:(target:SummarySourceTarget)=>void; type: ReportType }) {
  const {locale}=useI18n();
  ...
  const openOverviewSummary=(key:"currentAccountsBalance"|"currentInventoryValue"|"currentReceivable"|"currentPayable"|"sales"|"purchases"|"expenses"|"netOperatingResult")=>{if(!result)return;const details=result.overviewDetails;
    ...
    if(key==="sales"){const sale=detailTotal(details?.sales,"sale"),returns=detailTotal(details?.sales,"return");return setSummaryDetail({title:tr("صافي المبيعات"),total:reportNumber(result.summary.sales),rows:[{id:"overview-sales-total",label:tr("بيع"),value:sale,source:{kind:"report",reportType:"sales"}},{id:"overview-returns-total",label:tr("حركة تاريخية"),value:returns,source:{kind:"report",reportType:"sales"}}]})}
    if(key==="purchases"){const value=reportNumber(result.summary.purchases);return setSummaryDetail({title:tr("إجمالي المشتريات"),total:value,rows:[{id:"overview-purchases-total",label:tr("شراء"),value,source:{kind:"report",reportType:"purchases"}}]})}
    if(key==="expenses"){const value=reportNumber(result.summary.expenses);return setSummaryDetail({title:tr("إجمالي المصاريف"),total:value,rows:[{id:"overview-expenses-total",label:tr("مصروف"),value,source:{kind:"report",reportType:"expenses"}}]})}
    ...
    return setSummaryDetail({title:tr("صافي الربح"),total:reportNumber(result.summary.netOperatingResult),rows:[
      {id:"profit-sales",label:tr("صافي المبيعات"),value:sales,source:{kind:"report",reportType:"sales"}},
      {id:"profit-sales-cost",label:tr("تكلفة المبيعات"),value:-salesCost,source:{kind:"report",reportType:"sales"}},
      {id:"profit-expenses",label:tr("إجمالي المصاريف"),value:-expenses,source:{kind:"report",reportType:"expenses"}},
    ]});
  };
  if(type==="overview") {
