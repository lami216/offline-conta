...
type ManagedUser={id:string;name:string;username:string;isActive:boolean;permissions:string[]};
function UsersPermissions() {
  ...
  return <FramedSection title="المستخدمون والصلاحيات" className="users-permissions">
    <div className="users-permissions-layout">
      <fieldset className="permissions-pane"><legend>صلاحيات المستخدم</legend><div className="permission-toolbar"><span>اختر الصلاحيات الفعلية المطلوبة</span><button className="soft" type="button" onClick={()=>setPermissions(everythingSelected?[]:[...allPermissions])}>{everythingSelected?"إلغاء الكل":"تحديد الكل"}</button></div><div className="permissions-grid"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم الشاشة</th>{(["view","create","edit","delete"] as PermissionAction[]).map(action=><th key={action}>{actionLabels[action]}</th>)}<th>تحكم كامل</th></tr></thead><tbody>{permissionRows.map((row,i)=>{const keys=Object.values(row.actions),full=keys.every(key=>permissions.includes(key));return <tr key={row.name}><td>{number(i+1)}</td><td>{row.name}</td>{(["view","create","edit","delete"] as PermissionAction[]).map(action=>{const key=row.actions[action];return <td key={action}>{key&&<input aria-label={`${row.name} ${actionLabels[action]}`} type="checkbox" checked={permissions.includes(key)} onChange={event=>setPermissions(setPermission(permissions,key,event.target.checked))}/>}</td>})}<td><input aria-label={`${row.name} تحكم كامل`} type="checkbox" checked={full} onChange={event=>setPermissions(setRowFullControl(permissions,keys,event.target.checked))}/></td></tr>})}</tbody></table></div></fieldset>
      <aside className="user-admin-pane">
        <fieldset className="user-details-form"><legend>بيانات المستخدم</legend><div className="user-mode"><strong>{editing?"تعديل المستخدم":"مستخدم جديد"}</strong>{editing&&<span>{editing.username}</span>}</div><div className="user-fields"><label>اسم المستخدم<input dir="ltr" value={username} required autoComplete="off" onChange={event=>setUsername(event.target.value)}/></label><label>{editing?"كلمة المرور الجديدة (اختياري)":"كلمة المرور"}<input type="password" value={password} minLength={8} autoComplete="new-password" placeholder={editing?"اتركها فارغة للاحتفاظ بكلمة المرور الحالية":"8 أحرف على الأقل"} onChange={event=>setPassword(event.target.value)}/></label></div><div className="user-actions"><button className="soft" type="button" onClick={()=>select(null)}>مستخدم جديد</button><button className="primary" type="button" disabled={saving||!username.trim()||(!editing&&password.length<8)||(!!password&&password.length<8)} onClick={()=>void save()}>{saving?"جاري الحفظ…":"حفظ المستخدم"}</button>{editing&&<button className={active?"status-disable":"soft"} type="button" onClick={()=>setActive(value=>!value)}>{active?"تعطيل المستخدم":"تفعيل المستخدم"}</button>}</div>{error&&<div className="error" role="alert">{error}</div>}{success&&<div className="success" role="status">{success}</div>}</fieldset>
        <fieldset className="users-list"><legend>قائمة المستخدمين</legend><div className="users-list-scroll"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم المستخدم</th><th>الحالة</th><th>تعديل</th></tr></thead><tbody>{loading?<tr><td colSpan={4}>جاري تحميل المستخدمين…</td></tr>:loadError?<tr className="users-load-error"><td colSpan={4}>تعذر تحميل المستخدمين: {loadError} <button className="soft" onClick={()=>void load()}>إعادة المحاولة</button></td></tr>:users.length===0?<tr><td colSpan={4}>لا يوجد مستخدمون بعد</td></tr>:users.map((user,i)=><tr className={editing?.id===user.id?"selected":""} key={user.id}><td>{number(i+1)}</td><td dir="ltr">{user.username}</td><td><span className={user.isActive?"user-status active":"user-status"}>{user.isActive?"مفعل":"معطل"}</span></td><td><button className="soft" onClick={()=>select(user)}>تعديل</button></td></tr>)}</tbody></table></div></fieldset>
      </aside>
    </div>
  </FramedSection>
}
  ...
  return <div className={`settings-utility-row${selectedFile?" has-import-details":""}`}>
    ...
    <FramedSection title="الاستعادة والاستيراد" className="settings-import"><div className="import-head"><label className="file-button">اختيار ملف<input type="file" disabled={!canBackup&&!canImport} title={!canBackup&&!canImport?NO_ACCESS_TITLE:undefined} accept=".json,.conta.json,.db,.sqlite,application/json,application/vnd.sqlite3" onChange={e=>void chooseFile(e.target.files?.[0]??null)}/></label></div>
      ...
      {externalPreview&&<div className="source-preview"><div className="preview-heading"><div><b>نوع الملف: DataAcc SQLite</b><small>{selectedFile?.name}</small></div><span className="merge-badge">دمج آمن</span></div><div className="match-table"><div className="match-row match-head"><b>المجموعة</b><b>الإجمالي</b><b>جديد</b><b>مطابق</b><b>مراجعة</b></div>{externalPreview.groups?.filter(g=>g.count>0).map(g=><div className="match-row" key={g.key}><strong>{g.label}</strong><span>{number(g.count)}</span><span className="status-ready">{number(g.created)}</span><span>{number(g.matched)}</span><span className={g.review?"status-review":""}>{number(g.review)}</span></div>)}</div>
        ...
        <div className="policy-row"><label>تعارض المخزون<select value={stockPolicy} onChange={e=>setStockPolicy(e.target.value)}><option value="keep-current">الاحتفاظ برصيد الكرنة</option><option value="use-imported">استخدام الرصيد المستورد</option><option value="manual-resolution">حل يدوي</option></select></label><label>تعارض رصيد الحساب<select value={accountPolicy} onChange={e=>setAccountPolicy(e.target.value)}><option value="keep-current">الاحتفاظ برصيد الكرنة</option><option value="use-imported">استخدام الرصيد المستورد</option><option value="adjustment">تسجيل Adjustment بالفرق</option></select></label></div>
...
function SearchableSelect({ value, onChange, options, placeholder, searchPlaceholder, disabled = false, allowEmpty = false, floating = false, variant = "normal", ariaLabel }: {
  value: string; onChange: (value: string) => void; options: SelectOption[];
  placeholder: string; searchPlaceholder: string; disabled?: boolean; allowEmpty?: boolean; floating?: boolean; variant?: "normal" | "compact" | "pos-customer"; ariaLabel?: string;
}) {
  ...
  const closeSelect = useCallback(() => { setOpen(false); setQuery(""); setHighlightedIndex(null); setFloatingStyle({}); }, []);
  ...
  const list = <div ref={popover} className={`combobox-popover${floating ? " combobox-popover-floating" : ""}${variant === "compact" ? " combobox-popover-compact" : ""}${variant === "pos-customer" ? " combobox-popover-pos-customer" : ""}`} style={floating ? floatingStyle : undefined}>
    <label className="search"><Search /><input autoFocus value={query} placeholder={searchPlaceholder} onChange={e => { setQuery(e.target.value); setHighlightedIndex(null); }} onKeyDown={e => {
...
function ProductSearchPicker({ data, query, setQuery, onPick, mode = "sale", warehouseId, priceMode = "retail", collapseResultsWhenIdle = false, stockScope = "all-warehouses" }: {
  ...
  return <div className="product-picker product-search-grid">
    <label className="search compact-search"><Search /><input disabled={stockScope === "selected-warehouse" && !warehouseId} value={query} onChange={event => setQuery(event.target.value)} placeholder={stockScope === "selected-warehouse" && !warehouseId ? "اختر المخزن أولًا" : "ابحث بالاسم أو الكود أو الباركود"} /></label>
...
function Pos({
  ...
  const invoice = <FramedSection title="الفاتورة" className="invoice-card workspace-invoice">
    <InvoiceEditorToolbar number={editingDocument ? displayDocumentNumber(editingDocument) : String(data.nextDocumentSequences.sale)} newInvoice={() => { if (!dirty() || confirm("لديك تغييرات غير محفوظة. هل تريد بدء فاتورة جديدة؟")) resetEditor(); }} modes={<><button type="button" className="selection-option" aria-pressed={priceMode === "retail"} onClick={() => changePriceMode("retail")}>بيع الفرد</button><button type="button" className="selection-option" aria-pressed={priceMode === "wholesale"} onClick={() => changePriceMode("wholesale")}>بيع الجملة</button></>} scanner={<BarcodeScanner products={activeProducts(data.products)} onScan={add} enabled={scannerEnabled} onEnabledChange={setScannerEnabled} onError={setStockNotice} />} />
...
function QuickSupplier({ anchor, run, onDone, cancel }: { anchor: React.RefObject<HTMLButtonElement | null>; run: RunCommand; onDone: (id: string) => void; cancel: () => void }) {
  ...
  const rect = anchor.current?.getBoundingClientRect();
  const style: CSSProperties = rect ? { position: "fixed", zIndex: 1100, width: 250, top: rect.bottom + 5, left: Math.max(8, rect.right - 250) } : {};
  return createPortal(<form className="pos-quick-customer-popover" style={style} onSubmit={async event => { event.preventDefault(); const id = await run({ type: "party.create", partyType: "supplier", name, phone }, "تمت إضافة المورد"); onDone(id); }}><label>اسم المورد *<input autoFocus required value={name} onChange={e => setName(e.target.value)} /></label><label>رقم الهاتف <small>اختياري</small><input dir="ltr" value={phone} onChange={e => setPhone(e.target.value)} /></label><div><button className="primary">حفظ</button><button type="button" className="soft" onClick={cancel}>إلغاء</button></div></form>, document.body);
}
function Purchases({ data, run, openDoc, editRequest, clearEditRequest, requestPrint }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; editRequest: string | null; clearEditRequest: () => void; requestPrint: (id: string) => void }) {
  ...
  const baseline = useRef(""), quickSupplierButton = useRef<HTMLButtonElement>(null), editingDocument = editingDocumentId ? data.documents.find(document => document.id === editingDocumentId) ?? null : null;
...
function Expenses({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  return <section className="expense-workspace workspace-page">
    <div className="expense-grid">
      <FramedSection title="مصروف جديد" className="expense-form"><form className="expense-form-body" onSubmit={async event => { event.preventDefault(); const id = await run({ type: "expense.post", title, amount: val(amount), occurredAt: date, frequency, paymentMethod }, frequency === "once" ? "تم تسجيل المصروف" : "تم حفظ التذكير دون خصم"); setTitle(""); setAmount(""); if (frequency === "once") { setPaymentMethod(""); openDoc(id); } }}>
        <div className="expense-fields">
          ...
          {frequency === "once" && <label>وسيلة الدفع<select required value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}><option value="">اختر وسيلة الدفع</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></label>}
    ...
    {paying && <div className="modal-overlay" role="dialog" aria-modal="true"><form className="modal-card payment-dialog" onSubmit={async e => { e.preventDefault(); if (!recurringPaymentMethod) return; const recurring = data.recurringExpenses.find(r => r.id === paying)!; await run({ type: "expense.materialize", recurringId: paying, dueDate: recurring.currentDueDate, paymentMethod: recurringPaymentMethod }, "تم تسجيل دفع الاستحقاق"); setRecurringPaymentMethod(""); setPaying(null); }}><div className="modal-heading"><h3>تسجيل الدفع</h3><button type="button" className="icon" onClick={() => { setRecurringPaymentMethod(""); setPaying(null); }}><X /></button></div><label>تم الدفع من<select required value={recurringPaymentMethod} onChange={e => setRecurringPaymentMethod(e.target.value)}><option value="">اختر وسيلة الدفع</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></label><button className="primary" disabled={!recurringPaymentMethod}>تأكيد الدفع</button></form></div>}
  </section>;
}
...
function WarehouseAdmin({data,run,canDelete}:{data:BootstrapData;run:RunCommand;canDelete:boolean}){
  ...
  return <section className="workspace-page warehouse-admin"><FramedSection title="إضافة مخزن"><div className="warehouse-admin-create"><input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="اسم المخزن الجديد"/><button className="primary" disabled={!newName.trim()} onClick={async()=>{await run({type:"warehouse.create",name:newName},"تمت إضافة المخزن");setNewName("")}}><Plus/> إضافة مخزن</button></div></FramedSection><FramedSection title="إدارة المخازن" className="scroll-panel"><div className="erp-table-wrap"><table className="erp-table"><thead><tr><th>المخزن</th><th>الحالة</th><th>الاسم الجديد</th><th>إجراءات</th></tr></thead><tbody>{active.map(warehouse=><tr key={warehouse.id}><td className="name-cell">{warehouse.name}</td><td>{warehouse.isSalesDefault?"مخزن البيع الافتراضي":"نشط"}</td><td><input value={names[warehouse.id]??""} onChange={e=>setNames(current=>({...current,[warehouse.id]:e.target.value}))} placeholder={warehouse.name}/></td><td className="action-cell"><button className="soft" disabled={!names[warehouse.id]?.trim()} onClick={()=>void run({type:"warehouse.update",id:warehouse.id,name:names[warehouse.id]},"تم تعديل اسم المخزن")}>حفظ الاسم</button><button className="soft" disabled={warehouse.isSalesDefault} onClick={()=>void run({type:"warehouse.default",warehouseId:warehouse.id},"تم تحديد مخزن البيع الافتراضي")}>تعيين كافتراضي</button>{canDelete&&<button className="danger compact-delete" onClick={()=>{if(confirm("سيتم حذف المخزن الفارغ أو أرشفته عند وجود تاريخ مرتبط. هل تريد المتابعة؟"))void run({type:"warehouse.delete",id:warehouse.id},"تم حذف أو أرشفة المخزن")}}>حذف</button>}</td></tr>)}</tbody></table></div></FramedSection></section>;
}
