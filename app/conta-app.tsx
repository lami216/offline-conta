...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const active=activePaymentAccounts(data.paymentAccounts),visibleAccounts=data.paymentAccounts.filter(account=>account.isArchived!==true),archived=data.paymentAccounts.filter(account=>account.isArchived===true),name=(id:string)=>data.paymentAccounts.find(account=>account.id===id||account.code===id)?.name??id;
  const movementLabels:Record<string,string>={sale:tr("بيع"),purchase:tr("شراء"),expense:tr("مصروف"),"party-receipt":tr("سداد عميل"),"party-payment":tr("سداد مورد"),"transfer-in":tr("تحويل داخل"),"transfer-out":tr("تحويل خارج"),"manual-deposit":tr("إيداع"),"manual-withdrawal":tr("سحب"),"opening-balance":tr("رصيد بداية"),"opening-balance-correction":tr("تصحيح رصيد البداية"),"balance-correction":tr("تصحيح رصيد سابق")};
  ...
  const movementColumns=[{key:"date",type:"date" as const,get:(m:BootstrapData["financialMovements"][number])=>m.occurredAt},{key:"type",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>movementLabels[financialMovementKind(m.type)]??m.type},{key:"account",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>name(m.paymentMethod)},{key:"amount",type:"money" as const,get:(m:BootstrapData["financialMovements"][number])=>m.direction==="in"?m.amount:-m.amount},{key:"document",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>m.documentNumber}],{sort:movementSort,sortedRows:sortedMovements,toggle:toggleMovementSort}=useSortableRows(movements,movementColumns);
  const transferColumns=[{key:"date",type:"date" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.occurredAt},{key:"from",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>name(t.fromAccountId)},{key:"to",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>name(t.toAccountId)},{key:"amount",type:"money" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.amount},{key:"reference",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.number}],{sort:transferSort,sortedRows:sortedTransfers,toggle:toggleTransferSort}=useSortableRows(transfers,transferColumns);
