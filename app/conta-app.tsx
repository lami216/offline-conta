    ...
    <div className={`app-shell section-${view}`} dir={dir}>
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label={tr("التنقل الرئيسي")} onKeyDown={navigationKeyDown}>
          {nav.slice(0,1).map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} className={`nav nav-${n.id}`} onClick={()=>navigate(n.id)}><n.icon/><span>{tr(n.label)}</span></PermissionNavItem>)}
          ...
          {nav.slice(1).filter(n=>n.id!=="reports"&&n.id!=="settings"&&n.id!=="banks").map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} className={`nav nav-${n.id}`} onClick={()=>navigate(n.id)}><n.icon/><span>{tr(n.label)}</span></PermissionNavItem>)}
