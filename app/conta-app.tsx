...
import { invoiceShortcutAction, shortcutsForInvoice, type InvoiceKeyboardShortcut, type InvoiceShortcutAnchor, type InvoiceShortcutScope } from "./invoice-keyboard";
...
function ShortcutKeycaps({ keys }: { keys: readonly string[] }) {
  return <span className="keyboard-keycaps" dir="ltr">{keys.map((key, index) => <Fragment key={key}>{index > 0 && <span className="keyboard-key-plus">+</span>}<kbd>{key}</kbd></Fragment>)}</span>;
}

function KeyboardHelpOverlay({ scope, root, onClose }: { scope: InvoiceShortcutScope; root: React.RefObject<HTMLElement | null>; onClose: () => void }) {
  const shortcuts = shortcutsForInvoice(scope);
  const [positions, setPositions] = useState<Array<{ shortcut: InvoiceKeyboardShortcut; top: number; left: number }>>([]);
  useLayoutEffect(() => {
    const update = () => setPositions(shortcuts.flatMap(shortcut => {
      const anchor = root.current?.querySelector<HTMLElement>(`[data-keyboard-help="${shortcut.anchor}"]`);
      if (!anchor) return [];
      const rect = anchor.getBoundingClientRect(), badgeWidth = shortcut.keys.length > 1 ? 154 : 112;
      return [{ shortcut, top: Math.max(8, Math.min(window.innerHeight - 58, rect.top - 12)), left: Math.max(8, Math.min(window.innerWidth - badgeWidth - 8, rect.left + rect.width / 2 - badgeWidth / 2)) }];
    }));
    update(); window.addEventListener("resize", update); window.addEventListener("scroll", update, true);
    return () => { window.removeEventListener("resize", update); window.removeEventListener("scroll", update, true); };
  }, [root, shortcuts]);
  return createPortal(<div className="keyboard-help-overlay" role="dialog" aria-modal="true" aria-label="خريطة اختصارات الفاتورة" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <button type="button" className="keyboard-help-close" onClick={onClose} aria-label="إغلاق خريطة الاختصارات"><X /> إغلاق</button>
    {positions.map(({ shortcut, top, left }) => <div className="keyboard-help-badge" style={{ top, left }} key={shortcut.id}><ShortcutKeycaps keys={shortcut.keys} /><span>{shortcut.label}</span></div>)}
    <div className="keyboard-help-legend" aria-label="إرشادات التنقل"><span><ShortcutKeycaps keys={["Tab"]} /> التالي</span><span><ShortcutKeycaps keys={["Shift", "Tab"]} /> السابق</span><span><ShortcutKeycaps keys={["↑", "↓"]} /> التنقل داخل القوائم</span><span><ShortcutKeycaps keys={["Enter"]} /> اختيار</span><span><ShortcutKeycaps keys={["Esc"]} /> إغلاق</span></div>
  </div>, document.body);
}

function KeyboardHelpButton({ open, onToggle, buttonRef }: { open: boolean; onToggle: () => void; buttonRef: Ref<HTMLButtonElement> }) {
  return <button ref={buttonRef} type="button" className="keyboard-help-button" aria-label="عرض اختصارات لوحة المفاتيح" aria-pressed={open} onClick={onToggle}><Keyboard /> اختصارات</button>;
}
...
function Pos({
  ...
  const [query, setQuery] = useState(""),
    ...
    [keyboardHelpOpen, setKeyboardHelpOpen] = useState(false),
    quickButton = useRef<HTMLButtonElement>(null), keyboardHelpButton = useRef<HTMLButtonElement>(null), pageRef = useRef<HTMLElement>(null);
  ...
  const checkout = <FramedSection title="الدفع" className="workspace-checkout"><div className="checkout-layout"><div className="checkout-body"><div className="invoice-meta-row" aria-label="نوع الفاتورة"><button className="meta-option selection-option" aria-pressed={payment !== "note"} onClick={() => setPayment("")}><Banknote /><span><small>طريقة التحصيل</small><b>دفع مباشر</b></span></button><button className="meta-option selection-option secondary" aria-pressed={payment === "note"} onClick={() => setPayment("note")}><PencilLine /><span><small>نوع البيع</small><b>ملاحظة</b></span></button></div><span className="payment-label">{payment === "note" ? "العميل" : "طريقة الدفع"}</span><div data-keyboard-help="payment" className={`pos-payment-row${payment === "note" ? " note" : ""}`}>{payment !== "note" && <CompactPaymentSelector selectRef={paymentRef} accounts={data.paymentAccounts} value={payment} onChange={setPayment} />}<div className="pos-customer-compact">{customerSelect}</div><button ref={quickButton} type="button" className="pos-quick-customer-button" aria-label="إضافة العميل" onClick={() => setQuick(value => !value)}><Plus /><span>إضافة العميل</span></button></div>{quick && <QuickCustomer anchor={quickButton} run={run} cancel={() => setQuick(false)} onDone={id => { setPartyId(id); setQuick(false); }} />}<CheckoutInvoiceActions printAfterSave={printAfterSave} setPrintAfterSave={setPrintAfterSave} onDelete={voidSale} disabled={!editingDocument && !lines.length} /></div><div className="checkout-footer"><div className="total invoice-total"><span>الإجمالي</span><strong>{money(total)}</strong></div><button data-keyboard-help="submit" className="primary wide" disabled={!lines.length || !wh || (payment === "note" ? !partyId : !payment)} onClick={() => void submit()}>{editingDocumentId ? "حفظ التعديلات" : "إتمام البيع"}</button></div></div></FramedSection>;
  return <section ref={pageRef} className={`transaction-page${keyboardHelpOpen ? " keyboard-help-active" : ""}`} onKeyDownCapture={event => { if (!keyboardHelpOpen) return; event.preventDefault(); event.stopPropagation(); if (event.key === "Escape") { setKeyboardHelpOpen(false); window.requestAnimationFrame(() => keyboardHelpButton.current?.focus()); } }} onKeyDown={event => { const action = invoiceShortcutAction(event.nativeEvent, "sale"); if (action === "focus-product") { event.preventDefault(); productSearchRef.current?.focus(); } else if (action === "focus-payment") { event.preventDefault(); (payment === "note" ? customerRef.current : paymentRef.current)?.focus(); } else if (action === "submit") { event.preventDefault(); void submit(); } else if (!isTypingTarget(event.target) && event.key === "Escape") { /* Local popovers consume Escape before it reaches here. */ } }}><KeyboardHelpButton buttonRef={keyboardHelpButton} open={keyboardHelpOpen} onToggle={() => setKeyboardHelpOpen(value => !value)} />{keyboardHelpOpen && <KeyboardHelpOverlay scope="sale" root={pageRef} onClose={() => { setKeyboardHelpOpen(false); window.requestAnimationFrame(() => keyboardHelpButton.current?.focus()); }} />} {stockNotice && <div className="toast stock-toast">{stockNotice}</div>}<div className="transaction-workspace pos-workspace"><div className="workspace-discovery"><div data-keyboard-help="product-search"><FramedSection title="بحث المنتجات" className="search-panel"><SearchProducts inputRef={productSearchRef} data={data} query={query} setQuery={setQuery} onPick={add} mode="sale" warehouseId={wh?.id} priceMode={priceMode} stockScope="selected-warehouse" collapseResultsWhenIdle /></FramedSection></div><InvoiceQuickBrowser title="سجل الفواتير" docs={data.documents.filter(d => d.kind === "sale" && d.status === "posted")} openDoc={id => { const document = data.documents.find(item => item.id === id); if (document) loadDocument(document); }} /></div>{invoice}{checkout}</div></section>;
...
function Purchases({ data, run, openDoc, editRequest, clearEditRequest, requestPrint }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; editRequest: string | null; clearEditRequest: () => void; requestPrint: (id: string) => void }) {
  ...
  const [editingDocumentId, setEditingDocumentId] = useState<string | null>(null), [printAfterSave, setPrintAfterSave] = useState(false), [keyboardHelpOpen, setKeyboardHelpOpen] = useState(false);
  const baseline = useRef(""), quickSupplierButton = useRef<HTMLButtonElement>(null), productSearchRef = useRef<HTMLInputElement>(null), paymentRef = useRef<HTMLSelectElement>(null), supplierRef = useRef<HTMLButtonElement>(null), warehouseRef = useRef<HTMLButtonElement>(null), submittingRef = useRef(false), keyboardHelpButton = useRef<HTMLButtonElement>(null), pageRef = useRef<HTMLElement>(null), editingDocument = editingDocumentId ? data.documents.find(document => document.id === editingDocumentId) ?? null : null;
  ...
  return <section ref={pageRef} className={`transaction-page${keyboardHelpOpen ? " keyboard-help-active" : ""}`} onKeyDownCapture={event => { if (!keyboardHelpOpen) return; event.preventDefault(); event.stopPropagation(); if (event.key === "Escape") { setKeyboardHelpOpen(false); window.requestAnimationFrame(() => keyboardHelpButton.current?.focus()); } }} onKeyDown={event => { const action = invoiceShortcutAction(event.nativeEvent, "purchase"); if (action === "focus-product") { event.preventDefault(); productSearchRef.current?.focus(); } else if (action === "focus-payment") { event.preventDefault(); (payment === "note" ? supplierRef.current : paymentRef.current)?.focus(); } else if (action === "submit") { event.preventDefault(); void submit(); } }}><KeyboardHelpButton buttonRef={keyboardHelpButton} open={keyboardHelpOpen} onToggle={() => setKeyboardHelpOpen(value => !value)} />{keyboardHelpOpen && <KeyboardHelpOverlay scope="purchase" root={pageRef} onClose={() => { setKeyboardHelpOpen(false); window.requestAnimationFrame(() => keyboardHelpButton.current?.focus()); }} />} <div className="transaction-workspace purchase-workspace">
    <div className="workspace-discovery"><div data-keyboard-help="product-search"><FramedSection title="بحث المنتجات" className="search-panel"><SearchProducts inputRef={productSearchRef} data={data} query={query} setQuery={setQuery} onPick={pick} mode="purchase" warehouseId={warehouseId} collapseResultsWhenIdle /></FramedSection></div><InvoiceQuickBrowser title="سجل فواتير الشراء" docs={data.documents.filter(d => d.kind === "purchase" && d.status === "posted")} openDoc={id => { const document = data.documents.find(item => item.id === id); if (document) loadDocument(document); }} /></div>
