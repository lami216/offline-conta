...
function PermissionNavItem({allowed,active,onClick,className="",children}:{allowed:boolean;active:boolean;onClick:()=>void;className?:string;children:ReactNode}) {
  return <DisabledActionHint reasons={!allowed?[{id:"permission",label:NO_ACCESS_TITLE,kind:"permission"}]:[]}><button disabled={!allowed} aria-disabled={!allowed?"true":undefined} className={`${className}${allowed&&active?`${className?" ":""}active`:""}`} onClick={onClick}>{children}</button></DisabledActionHint>;
}
...
function GeneralSettings({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  return <div className="general-settings">
    ...
    {canBrand&&<DisabledActionHint reasons={[...(saving?[{id:"saving",label:"جاري الحفظ",kind:"busy" as const}]:[]),...(!dirty&&!saving?[{id:"dirty",label:"لا توجد تعديلات للحفظ",kind:"state" as const}]:[]),...(!branding.storeName.trim()?[{id:"store",label:"اسم المحل"}]:[])]}><button className="primary settings-save" disabled={saving||!dirty||!branding.storeName.trim()} onClick={()=>void save()}>{saving?"جاري الحفظ…":"حفظ الإعدادات"}</button></DisabledActionHint>}{notice&&<div className={notice==="تم حفظ الإعدادات"?"success":"error"}>{notice}</div>}
  </div>;
}
...
function DataSettings({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  return <div className={`settings-utility-row${selectedFile?" has-import-details":""}`}>
    <FramedSection title="النسخ الاحتياطي" className="settings-backup"><div><DisabledActionHint reasons={[...(busy?[{id:"busy",label:"هناك عملية بيانات جارية",kind:"busy" as const}]:[]),...(!canBackup?[{id:"permission",label:"لا تملك صلاحية إنشاء نسخة احتياطية",kind:"permission" as const}]:[])]}><button className="primary" disabled={!!busy||!canBackup} title={!canBackup?NO_ACCESS_TITLE:undefined} onClick={()=>{setBusy("backup");download().then(()=>setMessage("تم إنشاء النسخة وتنزيلها")).catch(e=>setFailure(e.message)).finally(()=>setBusy(""))}}>{busy==="backup"?"جاري الإنشاء…":"إنشاء وتنزيل"}</button></DisabledActionHint></div></FramedSection>
    <FramedSection title="الاستعادة والاستيراد" className="settings-import"><div className="import-head"><label className="file-button">اختيار ملف<input type="file" disabled={!canBackup&&!canImport} title={!canBackup&&!canImport?NO_ACCESS_TITLE:undefined} accept=".json,.conta.json,.db,.sqlite,application/json,application/vnd.sqlite3" onChange={e=>void chooseFile(e.target.files?.[0]??null)}/></label></div>
      ...
      {externalPreview&&<div className="source-preview"><div className="preview-heading"><div><b>نوع الملف: DataAcc SQLite</b><small>{selectedFile?.name}</small></div><span className="merge-badge">دمج آمن</span></div><div className="match-table"><div className="match-row match-head"><b>المجموعة</b><b>الإجمالي</b><b>جديد</b><b>مطابق</b><b>مراجعة</b></div>{externalPreview.groups?.filter(g=>g.count>0).map(g=><div className="match-row" key={g.key}><strong>{g.label}</strong><span>{number(g.count)}</span><span className="status-ready">{number(g.created)}</span><span>{number(g.matched)}</span><span className={g.review?"status-review":""}>{number(g.review)}</span></div>)}</div>
        ...
        <div className="final-actions"><small>سيتم إنشاء نسخة أمان تلقائية قبل أول مرحلة دمج. لن تُجمع الأرصدة أو كميات المخزون.</small><DisabledActionHint reasons={[...(busy?[{id:"busy",label:"هناك عملية بيانات جارية",kind:"busy" as const}]:[]),...(!canImport?[{id:"permission",label:"لا تملك صلاحية الاستيراد",kind:"permission" as const}]:[]),...(stockPolicy==="manual-resolution"?[{id:"policy",label:"اختر سياسة لحل تعارض المخزون"}]:[]),...(externalPreview.criticalConflicts?[{id:"conflict",label:"يوجد تعارض حرج يجب حله قبل الاستيراد"}]:[])]}><button className="primary" disabled={!!busy||!canImport||stockPolicy==="manual-resolution"||!!externalPreview.criticalConflicts} title={!canImport?NO_ACCESS_TITLE:undefined} onClick={()=>void importExternal()}>{busy==="import"?"جاري الاستيراد…":"تنفيذ الاستيراد"}</button></DisabledActionHint></div></div>}
...
export type DisabledReason={id:string;label:string;targetId?:string;kind?:"missing"|"busy"|"permission"|"state"};
export function DisabledActionHint({reasons,children}:{reasons:DisabledReason[];children:ReactNode}){
  const tooltipId=useId(),blocked=reasons.length>0,missing=reasons.filter(reason=>(reason.kind??"missing")==="missing");
  const inspect=(active:boolean)=>missing.forEach(reason=>{if(reason.targetId)document.getElementById(reason.targetId)?.classList.toggle("field-missing",active)});
  return <span className={`blocked-action${blocked?" is-blocked":""}`} tabIndex={blocked?0:undefined} aria-disabled={blocked||undefined} aria-describedby={blocked?tooltipId:undefined} onPointerEnter={()=>inspect(true)} onPointerLeave={()=>inspect(false)} onFocus={()=>inspect(true)} onBlur={()=>inspect(false)}>{children}{blocked&&<span id={tooltipId} role="tooltip" className="blocked-action-tooltip">{missing.length>0&&<b>ناقص:</b>}{reasons.map(reason=><span key={reason.id}>{missing.includes(reason)?"• ":""}{reason.label}</span>)}</span>}</span>;
}
export type MissingRequirement=DisabledReason;
const BlockedAction=DisabledActionHint;
...
function CheckoutInvoiceActions({ printAfterSave, setPrintAfterSave, onDelete, disabled }: { printAfterSave: boolean; setPrintAfterSave: (value: boolean) => void; onDelete: () => void; disabled?: boolean }) {
  return <div className="checkout-invoice-actions"><button type="button" className="print-toggle" aria-pressed={printAfterSave} onClick={() => setPrintAfterSave(!printAfterSave)}><span>{printAfterSave && <i />}</span>طباعة</button><DisabledActionHint reasons={disabled?[{id:"empty",label:"لا توجد فاتورة محفوظة لحذفها",kind:"state"}]:[]}><button type="button" className="invoice-void" disabled={disabled} onClick={onDelete}>حذف الفاتورة</button></DisabledActionHint></div>;
}
...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const accountColumns=useMemo(()=>[{key:"name",type:"text" as const,get:(a:PaymentAccount)=>a.name},{key:"status",type:"text" as const,get:(a:PaymentAccount)=>a.isActive?"نشط":"متوقف"},{key:"balance",type:"money" as const,get:(a:PaymentAccount)=>a.balance}],[]),{sort:accountSort,sortedRows:sortedAccounts,toggle:toggleAccountSort}=useSortableRows(active,accountColumns);
...
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  const query=normalizeSearch(q), filteredParties=data.parties.filter(p=>resolvePartyType(p)===partyType&&(!query||normalizeSearch(`${p.name} ${p.phone}`).includes(query)));
  ...
  const partyColumns=[{key:"name",type:"text" as const,get:(p:Party)=>p.name},{key:"phone",type:"text" as const,get:(p:Party)=>p.phone},{key:"trade",type:"money" as const,get:(p:Party)=>metricMap.get(p.id)?.total},{key:"profit",type:"money" as const,get:(p:Party)=>metricMap.get(p.id)?.grossProfit},{key:"balance",type:"money" as const,get:(p:Party)=>p.receivable-p.payable}],{sort:partySort,sortedRows:list,toggle:togglePartySort}=useSortableRows(filteredParties,partyColumns);
...
function WarehouseAdmin({data,run,canDelete}:{data:BootstrapData;run:RunCommand;canDelete:boolean}){
  ...
  const warehouseColumns=useMemo(()=>[{key:"name",type:"text" as const,get:(w:BootstrapData["warehouses"][number])=>w.name},{key:"status",type:"text" as const,get:(w:BootstrapData["warehouses"][number])=>w.isSalesDefault?"مخزن البيع":"نشط"}],[]),{sort:warehouseSort,sortedRows:sortedWarehouses,toggle:toggleWarehouseSort}=useSortableRows(active,warehouseColumns);
  return <section className="workspace-page warehouse-admin"><FramedSection title="إضافة مخزن"><div className="warehouse-admin-create"><input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="اسم المخزن الجديد"/><BlockedAction reasons={!newName.trim()?[{id:"name",label:"اسم المخزن"}]:[]}><button className="primary" disabled={!newName.trim()} onClick={async()=>{await run({type:"warehouse.create",name:newName},"تمت إضافة المخزن");setNewName("")}}><Plus/> إضافة مخزن</button></BlockedAction></div></FramedSection><FramedSection title="إدارة المخازن" className="scroll-panel"><div className="erp-table-wrap"><table className="erp-table"><thead><tr><SortableTableHeader column="name" label="المخزن" sort={warehouseSort} toggle={toggleWarehouseSort}/><SortableTableHeader column="status" label="الحالة" sort={warehouseSort} toggle={toggleWarehouseSort}/><th>الاسم الجديد</th><th>إجراءات</th></tr></thead><tbody>{sortedWarehouses.map(warehouse=><tr key={warehouse.id}><td className="name-cell">{warehouse.name}</td><td>{warehouse.isSalesDefault?"مخزن البيع":"نشط"}</td><td><input value={names[warehouse.id]??""} onChange={e=>setNames(current=>({...current,[warehouse.id]:e.target.value}))} placeholder={warehouse.name}/></td><td className="action-cell"><BlockedAction reasons={!names[warehouse.id]?.trim()?[{id:"name",label:"الاسم الجديد"}]:[]}><button className="soft" disabled={!names[warehouse.id]?.trim()} onClick={()=>void run({type:"warehouse.update",id:warehouse.id,name:names[warehouse.id]},"تم تعديل اسم المخزن")}>حفظ الاسم</button></BlockedAction><DisabledActionHint reasons={warehouse.isSalesDefault?[{id:"default",label:"هذا هو مخزن البيع الحالي",kind:"state"}]:[]}><button className="soft" disabled={warehouse.isSalesDefault} onClick={()=>void run({type:"warehouse.default",warehouseId:warehouse.id},"تم تعيين مخزن البيع")}>{warehouse.isSalesDefault?"مخزن البيع":"تعيين للبيع"}</button></DisabledActionHint>{canDelete&&<button className="danger compact-delete" onClick={async()=>{if(await confirmAction({message:"سيتم حذف المخزن الفارغ أو أرشفته عند وجود تاريخ مرتبط. هل تريد المتابعة؟",confirmLabel:"حذف المخزن",tone:"danger"}))void run({type:"warehouse.delete",id:warehouse.id},"تم حذف أو أرشفة المخزن")}}>حذف</button>}</td></tr>)}</tbody></table></div></FramedSection></section>;
}
...
function Warehouses({ data, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  return <section className="warehouse-workspace"><FramedSection title="جرد المخازن" className="inventory-panel">
    <div className="inventory-overview inventory-toolbar"><SearchableSelect value={wh} onChange={chooseWarehouse} placeholder="اختر المخزن" searchPlaceholder="ابحث عن مخزن" floating options={[{value:ALL_WAREHOUSES,label:"كل المخازن"},...availableWarehouses.map(w => ({ value: w.id, label: w.name }))]}/><CompactDateRange from={draftFrom} to={draftTo} allTime={hasInventoryView&&committedPeriod===null} onApply={commitPeriod} onAllTime={showAllInventory} onFromChange={setDraftFrom} onToChange={setDraftTo}/><DisabledActionHint reasons={!hasInventoryView?[{id:"view",label:"اعرض البيانات أولًا",kind:"state"}]:[]}><button className="soft inventory-print" disabled={!hasInventoryView} onClick={() => window.print()}><Printer /> طباعة الجرد</button></DisabledActionHint><div className="inventory-overview-metrics"><span><small>عدد المنتجات</small><b>{hasInventoryView?number(inventoryProducts.length):"—"}</b></span><span><small>إجمالي الكمية</small><b>{hasInventoryView?number(totalPieces):"—"}</b></span><span><small>قيمة المخزون</small><b>{hasInventoryView?number(inventoryValue):"—"}</b></span></div></div>
...
function Products({ data, run }: { data: BootstrapData; run: RunCommand }) {
  ...
  const productColumns=useMemo(()=>[{key:"name",type:"text" as const,get:(p:Product)=>p.name},{key:"price",type:"money" as const,get:(p:Product)=>p.piecePrice},{key:"wholesale",type:"money" as const,get:(p:Product)=>p.wholesalePrice},{key:"cost",type:"money" as const,get:(p:Product)=>p.lastPurchaseCost},{key:"stock",type:"number" as const,get:stockOf}],[]);
  const {sort,sortedRows:products,toggle:toggleSort}=useSortableRows(filteredProducts,productColumns);
  ...
  return <section className="workspace-page products-page">
    ...
    <FramedSection title="قائمة المنتجات" className="scroll-panel product-management">
      ...
      <table className="erp-table" aria-label="كل المنتجات"><colgroup><col style={{width:"10%"}}/><col style={{width:"22%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"12%"}}/><col style={{width:"23%"}}/></colgroup><thead><tr>
        <th>رقم</th><SortableTableHeader column="name" label="الاسم" sort={sort} toggle={toggleSort}/><SortableTableHeader column="price" label="سعر البيع" sort={sort} toggle={toggleSort}/><SortableTableHeader column="wholesale" label="سعر الجملة" sort={sort} toggle={toggleSort}/><SortableTableHeader column="cost" label="سعر آخر شراء" sort={sort} toggle={toggleSort}/><SortableTableHeader column="stock" label="المخزون" sort={sort} toggle={toggleSort}/><th>إجراءات</th>
      </tr></thead><tbody>
...
function MultiStockForm({
  ...
  return (
    <div className="form-stack stock-operation-panel">
      ...
      <BlockedAction reasons={[...(!from?[{id:"from",label:"اختر المخزن"}]:[]),...(mode==="transfer"&&!to?[{id:"to",label:"اختر مخزن الوجهة"}]:[]),...(!lines.length?[{id:"products",label:"أضف منتجًا"}]:[]),...(mode==="adjust"&&!reason.trim()?[{id:"reason",label:"أدخل سبب التصحيح"}]:[]),...(invalidAdjustment?[{id:"lines",label:"أدخل الكمية والتكلفة المطلوبة"}]:[])]}><button className="primary stock-primary-action" disabled={!from || (mode === "transfer" && !to) || !lines.length || (mode === "adjust" && (!reason.trim() || invalidAdjustment))} onClick={() => void submit()}>{mode === "transfer" ? "اعتماد التحويل" : "اعتماد التصحيح"}</button></BlockedAction>
    </div>
  );
}
function Transfer(p: {
...
function Linked({
  ...
  const columns=useMemo(()=>[{key:"kind",type:"text" as const,get:(d:DocumentRecord)=>kindLabels[d.kind]},{key:"number",type:"number" as const,get:(d:DocumentRecord)=>d.sequence??d.number},{key:"total",type:"money" as const,get:(d:DocumentRecord)=>d.total}],[]),{sort,sortedRows,toggle}=useSortableRows(linked,columns);
  return linked.length ? (<div className="panel"><Heading title="المعاملات المرتبطة" /><div className="erp-table-wrap"><table className="erp-table"><colgroup><col style={{width:"34%"}}/><col style={{width:"36%"}}/><col style={{width:"30%"}}/></colgroup><thead><tr><SortableTableHeader column="kind" label="المعاملة" sort={sort} toggle={toggle}/><SortableTableHeader column="number" label="المستند" sort={sort} toggle={toggle}/><SortableTableHeader column="total" label="المبلغ" sort={sort} toggle={toggle}/></tr></thead><tbody>{sortedRows.map(d => <tr key={d.id}><td>{kindLabels[d.kind]}</td><td dir="ltr">{displayDocumentNumber(d)}</td><td className="num-cell">{number(d.total)}</td></tr>)}</tbody></table></div></div>) : null;
}
