...
function SupportContactPage(){
  return <section className="support-contact" aria-labelledby="support-contact-heading"><div className="support-contact-copy"><span>الدعم الفني</span><h2 id="support-contact-heading">تواصل مع الدعم</h2><p>نحن هنا لمساعدتك. اختر وسيلة التواصل المناسبة.</p><div className="support-contact-methods"><a href="https://wa.me/22249823328" target="_blank" rel="noreferrer"><MessageCircle aria-hidden="true"/><span><small>واتساب</small><b dir="ltr">49823328</b></span></a><a href="tel:49823328"><Phone aria-hidden="true"/><span><small>اتصال هاتفي</small><b dir="ltr">49823328</b></span></a></div></div><div className="support-contact-image"><img src="/images/support-contact.png" alt="فريق الدعم الفني"/></div></section>;
}
...
function CompactPaymentSelector({ accounts: suppliedAccounts, value, onChange }: { accounts: PaymentAccount[]; value: string; onChange: (id: string) => void }) {
  return <div className="compact-payment" aria-label="طريقة الدفع"><PaymentAccountSelect accounts={suppliedAccounts} activeOnly value={value} onChange={onChange} placeholder="اختر وسيلة الدفع" aria-label="وسيلة الدفع"/></div>;
}

type PaymentAccountSelectProps={accounts:PaymentAccount[];value:string;onChange:(value:string)=>void;placeholder?:string;required?:boolean;disabled?:boolean;className?:string;activeOnly?:boolean;"aria-label"?:string};
function PaymentAccountSelect({accounts,value,onChange,placeholder="اختر وسيلة الدفع",required,disabled,className="",activeOnly=false,...props}:PaymentAccountSelectProps){const options=activeOnly?activePaymentAccounts(accounts):accounts;return <select {...props} className={`payment-account-select ${className}`.trim()} value={value??""} required={required} disabled={disabled} onChange={event=>onChange(event.target.value)}><option value="">{placeholder}</option>{options.map(account=><option key={account.id} value={account.id}>{account.name}</option>)}</select>}
