...
import { setRuntimeLocale, tr } from "./i18n/messages";
...
export default function ContaApp(){const {locale}=useI18n();setRuntimeLocale(locale);return <ConfirmationProvider><ContaAppContent/></ConfirmationProvider>}
    ...
    <div className={`app-shell section-${view}`} dir={dir}>
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label={tr("التنقل الرئيسي")} onKeyDown={navigationKeyDown}>
          {nav.slice(0,1).map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} className="nav" onClick={()=>navigate(n.id)}><n.icon/><span>{tr(n.label)}</span></PermissionNavItem>)}
          <div className="nav-menu" ref={invoiceMenuRef}><button className={invoiceNav.some(n=>n.id===view)?"nav active":"nav"} aria-expanded={invoiceMenu} onClick={()=>setInvoiceMenu(x=>!x)}><ReceiptText/><span>{tr("الفواتير")}</span><ChevronDown className="chevron"/></button>{invoiceMenu&&<div className="nav-popover">{invoiceNav.map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} onClick={()=>navigate(n.id)}><span>{tr(n.label)}</span></PermissionNavItem>)}</div>}</div>
          <div className="nav-menu" ref={warehouseMenuRef}><button className={warehouseNav.some(n=>n.id===view)?"nav active":"nav"} aria-expanded={warehouseMenu} onClick={()=>setWarehouseMenu(x=>!x)}><Boxes/><span>{tr("المخازن")}</span><ChevronDown className="chevron"/></button>{warehouseMenu&&<div className="nav-popover">{warehouseNav.map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} onClick={()=>navigate(n.id)}><span>{tr(n.label)}</span></PermissionNavItem>)}</div>}</div>
          ...
          {nav.slice(1).filter(n=>n.id!=="reports"&&n.id!=="settings"&&n.id!=="banks").map(n=><PermissionNavItem key={n.id} allowed={can(viewCapability[n.id])} active={view===n.id} className="nav" onClick={()=>navigate(n.id)}><n.icon/><span>{tr(n.label)}</span></PermissionNavItem>)}
          <div className="nav-menu bank-nav-menu" ref={bankMenuRef}><button className={view==="banks"?"nav active":"nav"} aria-expanded={bankMenu} onClick={()=>setBankMenu(open=>!open)}><Landmark/><span>{tr("البنوك")}</span><ChevronDown className="chevron"/></button>{bankMenu&&<div className="nav-popover bank-nav-popover">{bankNav.map(item=><PermissionNavItem key={item.id} allowed={can("banks.view")} active={view==="banks"&&bankTab===item.id} onClick={()=>{setBankTab(item.id);navigate("banks")}}><span>{tr(item.label)}</span></PermissionNavItem>)}</div>}</div>
          <div className="nav-menu report-nav-menu" ref={reportMenuRef}><button className={view==="reports"?"nav active":"nav"} aria-expanded={reportMenu} onClick={()=>setReportMenu(value=>!value)}><Receipt/><span>{tr("التقارير")}</span><ChevronDown className="chevron"/></button>{reportMenu&&<div className="nav-popover report-nav-popover">{reportOrder.map(id=><PermissionNavItem key={id} allowed={can("reports.view")} active={view==="reports"&&reportType===id} onClick={()=>{setReportType(id);navigate("reports")}}><span>{tr(reportNames[id])}</span></PermissionNavItem>)}</div>}</div>
          <div className="nav-menu settings-nav-menu" ref={settingsMenuRef}><button className={view==="settings"?"nav active":"nav"} aria-expanded={settingsMenu} onClick={()=>setSettingsMenu(value=>!value)}><SettingsIcon/><span>{tr("الإعدادات")}</span><ChevronDown className="chevron"/></button>{settingsMenu&&<div className="nav-popover">{([{id:"general",label:tr("إعدادات عامة")},{id:"users",label:tr("المستخدمون والصلاحيات")},{id:"data",label:tr("البيانات والنسخ الاحتياطي")},{id:"license",label:tr("رخصة التفعيل")},{id:"contact",label:tr("تواصل مع الدعم")}] as Array<{id:SettingsTab;label:string}>).map(item=><PermissionNavItem key={item.id} allowed={settingsAllowed(item.id)} active={view==="settings"&&settingsTab===item.id} onClick={()=>{setSettingsTab(item.id);if(item.id==="license"||item.id==="contact"){setView("settings");setSettingsMenu(false)}else navigate("settings")}}><span>{tr(item.label)}</span></PermissionNavItem>)}</div>}</div>
        </nav>
...
function SupportLicensePage({initialStatus,onActivated}:{initialStatus?:LicenseStatus;onActivated:()=>void|Promise<void>}){
  ...
  const refreshLicense=useCallback(()=>fetch("/api/license/status").then(readApiResponse).then(value=>setStatus(value as LicenseStatus)).catch(error=>setFailure(error instanceof Error?error.message:tr("تعذر التحقق من الترخيص"))),[]);
  ...
  const device=async()=>{setBusy("device");setFailure("");try{const value=await readApiResponse(await fetch("/api/license/device")) as {deviceId:string};setDeviceId(value.deviceId)}catch(error){setFailure(error instanceof Error?error.message:tr("تعذر استخراج رقم الجهاز. تواصل مع الدعم."))}finally{setBusy("")}};
...
function Pos({
  ...
  async function submit() {
    ...
    if (validation.warnings.length && !await confirmAction({title:tr("تنبيه البيع بأقل من التكلفة"),message:belowCostConfirmation(validation.warnings)})) return;
...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  return <section className="banks-workspace workspace-page"><FramedSection title={bankNav.find(item=>item.id===tab)?.label??tr("البنوك والحسابات")} className={`bank-panel${splitPanel?" bank-panel-centered-legend":""}`}><div className="bank-tab-stage">
...
function PartyMetricStrip({items,aggregate=false}:{items:PartyMetricStripItem[];aggregate?:boolean}) {
  return <div className={`party-trade-metrics${aggregate?" party-aggregate-metrics":""}`}>{items.map(item=><span key={tr(item.label)} className={`metric-${item.tone??"neutral"}`}><small>{tr(item.label)}</small>{item.count?<b>{number(item.value)}</b>:<MoneyValue value={item.value} tone={item.tone??"neutral"} className="financial-amount-summary"/>}</span>)}</div>;
}
function PartyAggregateMetrics({partyType,metrics,net}:{partyType:"customer"|"supplier";metrics:ReturnType<typeof partyAggregateMetrics>;net:number}) {
  const dues:PartyMetricStripItem={label:net>0?tr("إجمالي المستحقات لنا"):net<0?tr("إجمالي المستحقات علينا"):tr("إجمالي المستحقات"),value:Math.abs(net),tone:net>0?"positive":net<0?"negative":"neutral"};
  const customer=partyType==="customer", items:PartyMetricStripItem[]=customer
    ? [{label:tr("إجمالي ما اشتراه العملاء منا"),value:metrics.tradeTotal},{label:tr("إجمالي ما دفعه العملاء لنا"),value:metrics.cashIn,tone:"positive"},{label:tr("إجمالي ما دفعناه للعملاء"),value:metrics.cashOut,tone:"negative"},{label:tr("إجمالي الربح من مبيعات العملاء"),value:metrics.grossProfit,tone:metrics.grossProfit>0?"positive":metrics.grossProfit<0?"negative":"neutral"},dues]
    : [{label:tr("إجمالي مشترياتنا من الموردين"),value:metrics.tradeTotal},{label:tr("إجمالي ما دفعناه للموردين"),value:metrics.cashOut,tone:"negative"},{label:tr("إجمالي ما دفعه الموردون لنا"),value:metrics.cashIn,tone:"positive"},dues];
...
function ProductForm({ run, close, product, warehouses }: { run: RunCommand; close: () => void; product: Product | null; warehouses: BootstrapData["warehouses"] }) {
  ...
  return <form className="panel product-form" onSubmit={async event => { event.preventDefault(); const sensitive = product && (name.trim() !== product.name || (cost === "" ? null : val(cost)) !== product.pieceCost); const confirmed = sensitive ? await confirmAction({message:`أنت تغيّر بيانات أساسية للمنتج «${product.name}». هل تريد المتابعة؟`}) : true; if (!confirmed) return;
    await run({ type: product ? "product.update" : "product.create", id: product?.id, name, barcode, expiryDate, note, pieceCost: cost, piecePrice: price, wholesalePrice, openingStock, openingWarehouseId, confirmSensitive: confirmed }, product ? tr("تم تعديل المنتج") : tr("تم إنشاء المنتج")); close(); }}>
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const productOptions=data.products.map(p=>({value:p.id,label:`${p.name}${p.isArchived ? ` ${tr("(مؤرشف)")}` : ""}`,search:`${p.name} ${p.sku??""} ${p.barcode??""}`})),partyOptions=data.parties.filter(p=>resolvePartyType(p)===partyTypeFilter).map(p=>({value:p.id,label:p.name,search:`${p.name} ${p.phone??""}`})),accountName=(id:unknown)=>data.paymentAccounts.find(a=>a.id===id||a.code===id)?.name??(id?tr("حساب غير متاح"):"—"),table=reportTableModel(reportColumns(type,productId,groupBy),result),columns=table.columns,showDates=type!=="debts";
  ...
  const display=(key:string,value:unknown)=>{if(numericKeys.has(key))return number(reportNumber(value));if(key==="paymentMethod")return accountName(value);if(key==="movementType")return type==="party-ledger"?String(value??"—"):movementLabels[String(value)]??tr("عملية غير معروفة");if(key==="occurredAt"||key==="lastMovement")return value?formatDateTime(String(value)):"—";if(key==="date")return value?formatDate(String(value)):"—";if(typeof value==="number")return number(reportNumber(value));if(typeof value==="boolean")return value?tr("متكرر"):tr("مرة واحدة");return String(value??"—")};
...
function OfficialRecordSheet({presentation,branding}:{presentation:OfficialPresentation;branding:InvoiceBrandingSettings}){const style={fontFamily:invoiceFontFamilies[branding.nameFont],fontSize:`${branding.nameFontSize}pt`,fontWeight:branding.nameFontWeight} as CSSProperties,businessLine=[branding.storeAddress,branding.storePhone].filter(Boolean).join(" · "),registrationLine=[branding.registrationNumber&&`السجل التجاري: ${branding.registrationNumber}`,branding.taxNumber&&`الرقم الضريبي: ${branding.taxNumber}`].filter(Boolean).join(" · ");return <article className="official-record-sheet"><header className="official-record-header"><strong style={style}>{branding.storeName}</strong>{businessLine&&<span className="official-business-meta">{businessLine}</span>}{registrationLine&&<span className="official-business-meta">{registrationLine}</span>}<h1>{presentation.title}</h1><span>{presentation.meta.slice(0,2).map(x=>x[1]).join(" · ")}</span></header><div className="official-record-meta">{presentation.meta.slice(2).map(([label,value])=><span key={label}><small>{label}</small><b>{value}</b></span>)}</div>{presentation.columns&&<div className="official-record-table-wrap"><table className="official-record-table"><thead><tr>{presentation.columns.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{presentation.rows?.map((row,i)=><tr key={i}>{row.map((v,j)=><td key={j}>{v}</td>)}</tr>)}</tbody></table></div>}{presentation.totals&&<div className={`official-record-totals ${presentation.tone??"neutral"}`}>{presentation.totals.map(([label,value])=><span key={label}><small>{label}</small><strong>{value}</strong></span>)}</div>}<footer>{branding.footerNote&&<span>{branding.footerNote}</span>}<small>{tr("تم إنشاء هذا المستند بواسطة")} {APP_NAME}</small></footer></article>}
