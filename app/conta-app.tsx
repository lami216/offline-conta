...
type SummaryBreakdownRow={id:string;label:string;meta?:string;value:number;impact?:number};
type SummaryBreakdown={title:string;total:number;rows:SummaryBreakdownRow[];formula?:Array<{label:string;value:number}>};
function SummaryBreakdownDialog({detail,close}:{detail:SummaryBreakdown;close:()=>void}){
  const tone=(value:number):MoneyTone=>value>0?"positive":value<0?"negative":"neutral";
  return createPortal(<div className="modal-overlay" role="dialog" aria-modal="true" aria-label={detail.title}><div className="modal-card summary-breakdown-modal"><div className="summary-breakdown-head"><div><small>{tr("البيان")}</small><h2>{detail.title}</h2></div><button type="button" className="icon" aria-label={tr("إغلاق")} onClick={close}><X/></button></div>{detail.formula&&<div className="summary-breakdown-formula">{detail.formula.map((item,index)=><span key={`${item.label}-${index}`}><small>{item.label}</small><MoneyValue value={item.value} tone={tone(item.value)}/></span>)}</div>}<div className="erp-table-wrap summary-breakdown-table"><table className="erp-table"><thead><tr><th>{tr("البيان")}</th><th>{tr("المبلغ")}</th><th>{tr("الأثر في المجموع")}</th></tr></thead><tbody>{detail.rows.map(row=>{const impact=row.impact??row.value;return <tr key={row.id}><td><strong>{row.label}</strong>{row.meta&&<small className="summary-breakdown-note">{row.meta}</small>}</td><td className="num-cell"><MoneyValue value={row.value} tone={tone(row.value)}/></td><td className="num-cell"><MoneyValue value={impact} tone={tone(impact)}/></td></tr>})}</tbody><tfoot><tr><th colSpan={2}>{tr("المجموع")}</th><th className="num-cell"><MoneyValue value={detail.total} tone={tone(detail.total)} className="financial-amount-summary"/></th></tr></tfoot></table></div></div></div>,document.body);
}
...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const movementSummaryDetail=(direction:"in"|"out"):SummaryBreakdown=>{const source=direction==="in"?accountBreakdown.income:accountBreakdown.expenses;return{title:direction==="in"?tr("إجمالي المداخيل"):tr("إجمالي المصاريف"),total:direction==="in"?accountSummary.income:accountSummary.expenses,rows:source.map(row=>({id:`${direction}-${row.kind}`,label:movementLabels[row.kind]??row.kind,value:row.value,impact:row.value}))}};
  const accountBalanceDetail:SummaryBreakdown={title:tr("إجمالي الأرصدة الحالية"),total:accountSummary.currentBalance,rows:accountBreakdown.accounts.map(account=>({id:`account-${account.id}`,label:account.name,meta:account.isActive?tr("نشط"):tr("متوقف"),value:account.value}))};
  const debtDetail=(side:"owedToUs"|"weOwe"):SummaryBreakdown=>({title:side==="owedToUs"?tr("إجمالي المستحق لنا"):tr("إجمالي المستحق علينا"),total:accountSummary[side],rows:accountBreakdown.parties.map(party=>({id:`${side}-${party.id}`,label:party.name,meta:`${party.partyType==="customer"?tr("عميل"):tr("مورد")}${party.isArchived?` · ${tr("مؤرشف")}`:""}`,value:party[side]}))});
...
function Parties({ partyType, data, run, openParty, canEdit, canDelete }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void; canEdit:boolean; canDelete:boolean }) {
  const [q,setQ]=useState(""),[name,setName]=useState(""),[phone,setPhone]=useState(""),[editing,setEditing]=useState<Party|null>(null);
  ...
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    <FramedSection title={`إدارة ${plural}`} className="parties-controls"><CompactSearch value={q} onChange={setQ} placeholder={tr("بحث بالاسم أو الهاتف")}/>{canCreate&&<form className="parties-create" onSubmit={async e=>{e.preventDefault();await run({type:"party.create",partyType,name,phone},`تمت إضافة ${noun}`);setName("");setPhone("")}}><input required value={name} onChange={e=>setName(e.target.value)} placeholder={`اسم ${noun}`}/><input dir="ltr" value={phone} onChange={e=>setPhone(e.target.value)} placeholder={tr("رقم الهاتف")}/><button className="primary"><Plus/>  {tr("إضافة")} {noun}</button></form>}</FramedSection>
...
function PartyEditDialog({party,run,canEdit,canDelete,close}:{party:Party;run:RunCommand;canEdit:boolean;canDelete:boolean;close:()=>void}){
  ...
  const remove=async()=>{const message=hasBalance?tr("party.deleteBalanceConfirm",{party:noun,name:party.name,receivable:money(receivable),payable:money(payable)}):tr("party.deleteBalancedConfirm",{party:noun,name:party.name});if(!await confirmAction({message,confirmLabel:hasBalance?tr("شطب الرصيد والحذف"):tr("حذف"),tone:"danger"}))return;await run({type:"party.delete",id:party.id,writeOffBalance:hasBalance},partyType==="customer"?tr("party.deletedCustomer"):tr("party.deletedSupplier"));close()};
  return createPortal(<div className="modal-overlay section-parties" role="dialog" aria-modal="true" aria-label={`${tr("تعديل")} ${noun}`} onKeyDown={event=>{if(event.key==="Escape"){event.preventDefault();close()}}}><form className="modal-card party-edit-dialog" onSubmit={async event=>{event.preventDefault();if(!canEdit)return;await run({type:"party.update",id:party.id,name,phone},partyType==="customer"?tr("party.savedCustomer"):tr("party.savedSupplier"));close()}}><div className="modal-heading"><h3>{tr("تعديل")} {noun}</h3><button type="button" className="icon" aria-label={tr("إغلاق")} onClick={close}><X/></button></div><label>{tr("الاسم")}<input autoFocus required disabled={!canEdit} value={name} onChange={event=>setName(event.target.value)}/></label><label>{tr("رقم الهاتف")}<input dir="ltr" disabled={!canEdit} value={phone} onChange={event=>setPhone(event.target.value)}/></label>{hasBalance&&<div className="party-delete-warning"><strong>{tr("تنبيه")}</strong><span>{tr("party.currentBalanceWillBeCleared",{receivable:money(receivable),payable:money(payable)})}</span></div>}<div className="dialog-actions party-edit-actions">{canEdit&&<button className="primary" disabled={!name.trim()}>{tr("حفظ التعديل")}</button>}{canDelete&&<button type="button" className="danger" onClick={()=>void remove()}>{tr("حذف")} {noun}</button>}</div></form></div>,document.body)
}
function PartyPage({party,data,openDoc,run}:{party:Party;data:BootstrapData;openDoc:(id:string)=>void;run:RunCommand}) {
  ...
  const canCreatePayment=canUseCapability(data.principal,customer?"customers.collect":"suppliers.pay"),canEditPayment=canUseCapability(data.principal,customer?"customers.collect.edit":"suppliers.pay.edit"),canDeletePayment=canUseCapability(data.principal,customer?"customers.collect.delete":"suppliers.pay.delete");
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const excludedProfitMeta=tr("لا يدخل مباشرة في معادلة صافي الربح");
  const openOverviewSummary=(key:"currentAccountsBalance"|"currentInventoryValue"|"currentReceivable"|"currentPayable"|"sales"|"purchases"|"expenses"|"netOperatingResult")=>{if(!result)return;const details=result.overviewDetails;
    ...
    if(key==="sales"){const sale=detailTotal(details?.sales,"sale"),returns=detailTotal(details?.sales,"return");return setSummaryDetail({title:tr("صافي المبيعات"),total:reportNumber(result.summary.sales),rows:[{id:"overview-sales-total",label:tr("بيع"),value:sale,impact:sale},{id:"overview-returns-total",label:tr("حركة تاريخية"),value:Math.abs(returns),impact:returns}]})}
    if(key==="purchases"){const value=reportNumber(result.summary.purchases);return setSummaryDetail({title:tr("إجمالي المشتريات"),total:value,rows:[{id:"overview-purchases-total",label:tr("شراء"),value,impact:value}]})}
    if(key==="expenses"){const value=reportNumber(result.summary.expenses);return setSummaryDetail({title:tr("إجمالي المصاريف"),total:value,rows:[{id:"overview-expenses-total",label:tr("مصروف"),value,impact:value}]})}
    const sales=reportNumber(result.summary.sales),salesCost=reportNumber(result.summary.salesCost),expenses=reportNumber(result.summary.expenses),purchases=reportNumber(result.summary.purchases),purchaseCashPaid=reportNumber(result.summary.purchaseCashPaid),withdrawals=reportNumber(result.summary.manualWithdrawals),deposits=reportNumber(result.summary.manualDeposits),partyReceipts=reportNumber(result.summary.partyReceipts),partyPayments=reportNumber(result.summary.partyPayments);
    return setSummaryDetail({title:tr("صافي الربح"),total:reportNumber(result.summary.netOperatingResult),formula:[{label:tr("صافي المبيعات"),value:sales},{label:tr("تكلفة المبيعات"),value:-salesCost},{label:tr("إجمالي المصاريف"),value:-expenses}],rows:[
      {id:"profit-sales",label:tr("صافي المبيعات"),value:sales,impact:sales},
      {id:"profit-sales-cost",label:tr("تكلفة المبيعات"),value:salesCost,impact:-salesCost},
      {id:"profit-expenses",label:tr("إجمالي المصاريف"),value:expenses,impact:-expenses},
      {id:"profit-purchases",label:tr("إجمالي المشتريات"),meta:excludedProfitMeta,value:purchases,impact:0},
      {id:"profit-purchase-cash",label:tr("المدفوع نقدًا للمشتريات"),meta:excludedProfitMeta,value:purchaseCashPaid,impact:0},
      {id:"profit-withdrawals",label:tr("السحب اليدوي"),meta:excludedProfitMeta,value:withdrawals,impact:0},
      {id:"profit-deposits",label:tr("الإيداع اليدوي"),meta:excludedProfitMeta,value:deposits,impact:0},
      {id:"profit-party-receipts",label:tr("سداد عميل"),meta:excludedProfitMeta,value:partyReceipts,impact:0},
      {id:"profit-party-payments",label:tr("سداد مورد"),meta:excludedProfitMeta,value:partyPayments,impact:0},
    ]});
  };
  if(type==="overview") {
