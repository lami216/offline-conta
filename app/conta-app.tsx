...
import { isModifiedEnter, isTypingTarget } from "./keyboard";
...
function Pos({
  ...
  return <section className="transaction-page" onKeyDown={event => { if (event.key === "F2") { event.preventDefault(); productSearchRef.current?.focus(); } else if (event.key === "F4") { event.preventDefault(); (payment === "note" ? customerRef.current : paymentRef.current)?.focus(); } else if (isModifiedEnter(event.nativeEvent)) { event.preventDefault(); void submit(); } else if (!isTypingTarget(event.target) && event.key === "Escape") { /* Local popovers consume Escape before it reaches here. */ } }}>{stockNotice && <div className="toast stock-toast">{stockNotice}</div>}<div className="transaction-workspace pos-workspace"><div className="workspace-discovery"><FramedSection title="بحث المنتجات" className="search-panel"><SearchProducts inputRef={productSearchRef} data={data} query={query} setQuery={setQuery} onPick={add} mode="sale" warehouseId={wh?.id} priceMode={priceMode} stockScope="selected-warehouse" collapseResultsWhenIdle /></FramedSection><InvoiceQuickBrowser title="سجل الفواتير" docs={data.documents.filter(d => d.kind === "sale" && d.status === "posted")} openDoc={id => { const document = data.documents.find(item => item.id === id); if (document) loadDocument(document); }} /></div>{invoice}{checkout}</div></section>;
...
function Purchases({ data, run, openDoc, editRequest, clearEditRequest, requestPrint }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; editRequest: string | null; clearEditRequest: () => void; requestPrint: (id: string) => void }) {
  ...
  return <section className="transaction-page" onKeyDown={event => { if (event.key === "F2") { event.preventDefault(); productSearchRef.current?.focus(); } else if (event.key === "F4") { event.preventDefault(); (payment === "note" ? supplierRef.current : paymentRef.current)?.focus(); } else if (isModifiedEnter(event.nativeEvent)) { event.preventDefault(); void submit(); } }}><div className="transaction-workspace purchase-workspace">
