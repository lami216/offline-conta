...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const openOverviewSummary=(key:"currentAccountsBalance"|"currentInventoryValue"|"currentReceivable"|"currentPayable"|"sales"|"purchases"|"expenses"|"netOperatingResult")=>{if(!result)return;const details=result.overviewDetails;
    if(key==="currentAccountsBalance")return setSummaryDetail({title:tr("إجمالي الأرصدة الحالية"),total:reportNumber(result.summary.currentAccountsBalance),rows:(result.bankAccounts??[]).map(account=>({id:`overview-account-${account.id}`,label:account.name,value:reportNumber(account.balance)}))});
    ...
    if(key==="sales"){const sale=detailTotal(details?.sales,"sale"),returns=detailTotal(details?.sales,"return");return setSummaryDetail({title:tr("صافي المبيعات"),total:reportNumber(result.summary.sales),rows:[{id:"overview-sales-total",label:tr("بيع"),value:sale},{id:"overview-returns-total",label:tr("حركة تاريخية"),value:returns}]})}
    if(key==="purchases"){const value=reportNumber(result.summary.purchases);return setSummaryDetail({title:tr("إجمالي المشتريات"),total:value,rows:[{id:"overview-purchases-total",label:tr("شراء"),value}]})}
    if(key==="expenses"){const value=reportNumber(result.summary.expenses);return setSummaryDetail({title:tr("إجمالي المصاريف"),total:value,rows:[{id:"overview-expenses-total",label:tr("مصروف"),value}]})}
    ...
    return setSummaryDetail({title:tr("صافي الربح"),total:reportNumber(result.summary.netOperatingResult),rows:[
      {id:"profit-sales",label:tr("صافي المبيعات"),value:sales},
      {id:"profit-sales-cost",label:tr("تكلفة المبيعات"),value:-salesCost},
      {id:"profit-expenses",label:tr("إجمالي المصاريف"),value:-expenses},
    ]});
  };
  if(type==="overview") {
