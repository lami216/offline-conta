...
function Reports({ data, openDoc, openSource, type, sourceRequest, clearSourceRequest }: { data: BootstrapData; openDoc: (id: string) => void; openSource:(target:SummarySourceTarget)=>void; type: ReportType; sourceRequest:{reportType:ReportType;period:CommittedPeriod}|null; clearSourceRequest:()=>void }) {
  ...
  const runReport=async(period:CommittedPeriod)=>{if(type==="party-ledger"&&!partyId){setReportError(partyTypeFilter==="customer"?tr("اختر العميل أولاً"):tr("اختر المورد أولاً"));return}reportRequest.current?.abort();const controller=new AbortController();reportRequest.current=controller;setBusy(true);setReportError("");const q=new URLSearchParams({type,unpaged:"true"});if(type!=="debts")Object.entries(reportDateQuery(period===null,period?.from??"",period?.to??"")).forEach(([key,value])=>q.set(key,value));const add=(key:string,value:string)=>{if(value)q.set(key,value)};if(["sales","purchases","product-sales","profit","stock"].includes(type)){add("categoryId",categoryId);add("productId",productId);}if(type==="party-ledger")add("partyId",partyId);if(["sales","purchases","financial","expenses"].includes(type))add("paymentAccountId",accountId);if(type==="profit")add("groupBy",groupBy);if(type==="stock")add("movementType",movementType);if(type==="financial")add("direction",direction);if(type==="debts"){add("debtSide",debtSide);add("search",search)}try{const response=await fetch(`/api/reports?${q}`,{signal:controller.signal}),json=await response.json();if(!response.ok)throw new Error(json.error);if(!controller.signal.aborted)setResult(json)}catch(error){if((error as Error).name!=="AbortError")setReportError(translateApiError(locale,error instanceof Error?error.message:tr("تعذر إنشاء التقرير")))}finally{if(reportRequest.current===controller)setBusy(false)}};
  ...
  const applyAllTime=()=>{setDraftFrom("");setDraftTo("");setCommittedPeriod(null);setCategoryId("");setProductId("");setAccountId("");setMovementType("");setDirection("");setDebtSide("");setSearch("");void runReport(null)};
  // A source-navigation request carries the exact period used by the aggregate that produced the number.
  // eslint-disable-next-line react-hooks/exhaustive-deps, react-hooks/set-state-in-effect
  useEffect(()=>{if(!sourceRequest||sourceRequest.reportType!==type)return;const period=sourceRequest.period;if(period===null){setDraftFrom("");setDraftTo("");setCommittedPeriod(null);void runReport(null)}else{setDraftFrom(period.from);setDraftTo(period.to);setCommittedPeriod(period);void runReport(period)}clearSourceRequest()},[sourceRequest]);
  ...
  const reportSortColumns=columns.map(([key,,columnType])=>({key,type:columnType,get:(row:ReportResponse["rows"][number])=>row[key]}));
