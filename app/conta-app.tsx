    ...
    <div className={`app-shell section-${view}`} dir={dir}>
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label={tr("التنقل الرئيسي")} onKeyDown={navigationKeyDown}>
          ...
          <div className="nav-menu nav-accounts bank-nav-menu" ref={bankMenuRef}><button className={view==="banks"?"nav active":"nav"} aria-expanded={bankMenu} onClick={()=>setBankMenu(open=>!open)}><Landmark/><span>{tr("البنوك")}</span><ChevronDown className="chevron"/></button>{bankMenu&&<div className="nav-popover bank-nav-popover">{bankNav.map(item=><PermissionNavItem key={item.id} allowed={can(bankTabCapability[item.id])} active={view==="banks"&&effectiveBankTab===item.id} onClick={()=>void (async()=>{if(item.id===effectiveBankTab||await navigate("banks",{replaceEditor:true}))setBankTab(item.id)})()}><span>{tr(item.label)}</span></PermissionNavItem>)}</div>}</div>
