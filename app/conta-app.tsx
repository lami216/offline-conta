...
type SummaryBreakdownRow={id:string;label:string;meta?:string;value:number};
...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const movementSummaryDetail=(direction:"in"|"out"):SummaryBreakdown=>{const source=direction==="in"?accountBreakdown.income:accountBreakdown.expenses;return{title:direction==="in"?tr("إجمالي المداخيل"):tr("إجمالي المصاريف"),total:direction==="in"?accountSummary.income:accountSummary.expenses,rows:source.map((row,index)=>({id:`${direction}-${row.kind}-${row.movement?.id??`zero-${index}`}`,label:movementLabels[row.kind]??row.kind,meta:row.movement?[name(row.movement.paymentMethod),row.movement.documentNumber,formatDate(row.movement.occurredAt)].filter(Boolean).join(" · "):"—",value:row.value}))}};
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const overviewDocumentRows=(rows:OverviewBreakdownRow[]|undefined,label:(row:OverviewBreakdownRow)=>string,value:(row:OverviewBreakdownRow)=>number=(row)=>row.value):SummaryBreakdownRow[]=>(rows??[]).map((row,index)=>({id:`overview-${row.kind}-${row.id}-${index}`,label:label(row),meta:[row.number,formatDate(row.occurredAt)].filter(Boolean).join(" · "),value:value(row)}));
  const openOverviewSummary=(key:"currentAccountsBalance"|"currentInventoryValue"|"currentReceivable"|"currentPayable"|"sales"|"purchases"|"expenses"|"netOperatingResult")=>{if(!result)return;const details=result.overviewDetails;
    if(key==="currentAccountsBalance")return setSummaryDetail({title:tr("إجمالي الأرصدة الحالية"),total:reportNumber(result.summary.currentAccountsBalance),rows:(result.bankAccounts??[]).map(account=>({id:`overview-account-${account.id}`,label:account.name,meta:"—",value:reportNumber(account.balance)}))});
    if(key==="currentInventoryValue")return setSummaryDetail({title:tr("إجمالي قيمة المخزون"),total:reportNumber(result.summary.currentInventoryValue),rows:(result.warehouseValues??[]).map(warehouse=>({id:`overview-warehouse-${warehouse.id}`,label:warehouse.name,meta:warehouse.archived?tr("مؤرشف"):"—",value:reportNumber(warehouse.value)}))});
    ...
    if(key==="sales"){const rows=overviewDocumentRows(details?.sales,row=>row.kind==="return"?tr("حركة تاريخية"):tr("فاتورة بيع"));if(!details?.sales.some(row=>row.kind==="sale"))rows.push({id:"overview-sales-zero",label:tr("فاتورة بيع"),meta:"—",value:0});if(!details?.sales.some(row=>row.kind==="return"))rows.push({id:"overview-returns-zero",label:tr("حركة تاريخية"),meta:"—",value:0});return setSummaryDetail({title:tr("صافي المبيعات"),total:reportNumber(result.summary.sales),rows})}
    if(key==="purchases"){const rows=overviewDocumentRows(details?.purchases,()=>tr("فاتورة شراء"));if(!rows.length)rows.push({id:"overview-purchases-zero",label:tr("فاتورة شراء"),meta:"—",value:0});return setSummaryDetail({title:tr("إجمالي المشتريات"),total:reportNumber(result.summary.purchases),rows})}
    if(key==="expenses"){const rows=overviewDocumentRows(details?.expenses,()=>tr("فاتورة مصروفات"));if(!rows.length)rows.push({id:"overview-expenses-zero",label:tr("فاتورة مصروفات"),meta:"—",value:0});return setSummaryDetail({title:tr("إجمالي المصاريف"),total:reportNumber(result.summary.expenses),rows})}
    const profitRows=overviewDocumentRows(details?.salesProfit,row=>row.kind==="return"?tr("حركة تاريخية"):tr("ربح المبيعات"),row=>reportNumber((row as {profit?:number}).profit));
    const expenseRows=overviewDocumentRows(details?.expenses,()=>tr("إجمالي المصاريف"),row=>-reportNumber(row.value)).map(row=>({...row,id:`net-${row.id}`}));
    if(!profitRows.length)profitRows.push({id:"net-sales-zero",label:tr("ربح المبيعات"),meta:"—",value:0});
    if(!expenseRows.length)expenseRows.push({id:"net-expenses-zero",label:tr("إجمالي المصاريف"),meta:"—",value:0});
    return setSummaryDetail({title:tr("صافي الربح"),total:reportNumber(result.summary.netOperatingResult),formula:[{label:tr("ربح المبيعات"),value:reportNumber(result.summary.salesProfit)},{label:tr("إجمالي المصاريف"),value:-reportNumber(result.summary.expenses)}],rows:[...profitRows,...expenseRows]});
  };
  if(type==="overview") {
