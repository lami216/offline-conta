...
import { invoiceShortcutAction, shortcutForAction, type InvoiceShortcutAction, type InvoiceShortcutScope } from "./invoice-keyboard";
...
function SearchableSelect({ value, onChange, options, placeholder, searchPlaceholder, disabled = false, allowEmpty = false, floating = false, variant = "normal", ariaLabel, triggerRef }: {
  ...
  const list = <div ref={popover} className={`combobox-popover${floating ? " combobox-popover-floating" : ""}${variant === "compact" ? " combobox-popover-compact" : ""}${variant === "pos-customer" ? " combobox-popover-pos-customer" : ""}`} style={floating ? floatingStyle : undefined}>
    ...
    <div id={listId} className="combobox-results" role="listbox" aria-labelledby={searchId}>
      ...
      {matches.map((option, index) => <button id={`${listId}-option-${index}`} type="button" role="option" aria-selected={highlightedIndex === index} className={[option.value === value && "selected", highlightedIndex === index && "highlighted"].filter(Boolean).join(" ")} key={option.value} onMouseEnter={() => setHighlightedIndex(index)} onMouseLeave={() => setHighlightedIndex(null)} onClick={() => choose(option.value)}>{option.label}</button>)}
...
function ShortcutHintAnchor({ scope, action, visible, children, className = "" }: { scope: InvoiceShortcutScope; action: InvoiceShortcutAction; visible: boolean; children: ReactNode; className?: string }) {
  const shortcut = shortcutForAction(scope, action);
  return <div className={`keyboard-shortcut-anchor ${className}`.trim()}>{children}{visible && shortcut && <kbd className="keyboard-shortcut-badge" aria-hidden="true" dir="ltr">{shortcut.keys[0].join("+")}</kbd>}</div>;
}

function KeyboardHelpButton({ open, onToggle, buttonRef }: { open: boolean; onToggle: () => void; buttonRef: Ref<HTMLButtonElement> }) {
  return <button ref={buttonRef} type="button" className="keyboard-help-button" aria-label="عرض اختصارات لوحة المفاتيح" aria-pressed={open} onClick={onToggle}><Keyboard /> <kbd>F1</kbd> اختصارات</button>;
}
...
function Pos({
  ...
  const [query, setQuery] = useState(""),
    ...
    [keyboardHintsVisible, setKeyboardHintsVisible] = useState(false),
    quickButton = useRef<HTMLButtonElement>(null), keyboardHelpButton = useRef<HTMLButtonElement>(null);
  ...
  const invoice = <FramedSection title="الفاتورة" className="invoice-card workspace-invoice">
    <InvoiceEditorToolbar number={editingDocument ? displayDocumentNumber(editingDocument) : String(data.nextDocumentSequences.sale)} newInvoice={newInvoice} scope="sale" hintsVisible={keyboardHintsVisible} modes={<><button type="button" className="selection-option" aria-pressed={priceMode === "retail"} onClick={() => changePriceMode("retail")}>بيع الفرد</button><button type="button" className="selection-option" aria-pressed={priceMode === "wholesale"} onClick={() => changePriceMode("wholesale")}>بيع الجملة</button></>} scanner={<BarcodeScanner products={activeProducts(data.products)} onScan={add} enabled={scannerEnabled} onEnabledChange={setScannerEnabled} onError={setStockNotice} />} />
  ...
  const checkout = <FramedSection title="الدفع" className="workspace-checkout"><div className="checkout-layout"><div className="checkout-body"><div className="invoice-meta-row" aria-label="نوع الفاتورة"><ShortcutHintAnchor scope="sale" action="select-direct" visible={keyboardHintsVisible}><button className="meta-option selection-option" aria-pressed={payment !== "note"} onClick={() => setPayment("")}><Banknote /><span><small>طريقة التحصيل</small><b>دفع مباشر</b></span></button></ShortcutHintAnchor><ShortcutHintAnchor scope="sale" action="select-note" visible={keyboardHintsVisible}><button className="meta-option selection-option secondary" aria-pressed={payment === "note"} onClick={() => setPayment("note")}><PencilLine /><span><small>نوع البيع</small><b>ملاحظة</b></span></button></ShortcutHintAnchor></div><span className="payment-label">{payment === "note" ? "العميل" : "طريقة الدفع"}</span><div className={`pos-payment-row${payment === "note" ? " note" : ""}`}>{payment !== "note" && <ShortcutHintAnchor scope="sale" action="focus-payment" visible={keyboardHintsVisible}><CompactPaymentSelector selectRef={paymentRef} accounts={data.paymentAccounts} value={payment} onChange={setPayment} /></ShortcutHintAnchor>}<ShortcutHintAnchor scope="sale" action="focus-party" visible={keyboardHintsVisible} className="pos-customer-compact">{customerSelect}</ShortcutHintAnchor><ShortcutHintAnchor scope="sale" action="toggle-party-quick" visible={keyboardHintsVisible}><button ref={quickButton} type="button" className="pos-quick-customer-button" aria-label="إضافة العميل" onClick={() => setQuick(value => !value)}><Plus /><span>إضافة العميل</span></button></ShortcutHintAnchor></div>{quick && <QuickCustomer anchor={quickButton} run={run} cancel={() => setQuick(false)} onDone={id => { setPartyId(id); setQuick(false); }} />}<CheckoutInvoiceActions scope="sale" hintsVisible={keyboardHintsVisible} printAfterSave={printAfterSave} setPrintAfterSave={setPrintAfterSave} onDelete={voidSale} disabled={!editingDocument && !lines.length} /></div><div className="checkout-footer"><div className="total invoice-total"><span>الإجمالي</span><strong>{money(total)}</strong></div><ShortcutHintAnchor scope="sale" action="submit" visible={keyboardHintsVisible}><button className="primary wide" disabled={!lines.length || !wh || (payment === "note" ? !partyId : !payment)} onClick={() => void submit()}>{editingDocumentId ? "حفظ التعديلات" : "إتمام البيع"}</button></ShortcutHintAnchor></div></div></FramedSection>;
  return <section className={`transaction-page${keyboardHintsVisible ? " keyboard-hints-active" : ""}`} onKeyDown={event => { if (event.defaultPrevented) return; const action = invoiceShortcutAction(event.nativeEvent, "sale"); if (!action) { if (event.key === "Escape" && keyboardHintsVisible) setKeyboardHintsVisible(false); return; } if ((action === "select-direct" || action === "select-note") && isTypingTarget(event.target)) return; event.preventDefault(); if (action === "toggle-hints") setKeyboardHintsVisible(value => !value); else if (action === "focus-product") productSearchRef.current?.focus(); else if (action === "focus-party") customerRef.current?.click(); else if (action === "focus-payment") (payment === "note" ? customerRef.current : paymentRef.current)?.focus(); else if (action === "toggle-party-quick") quickButton.current?.click(); else if (action === "toggle-print") setPrintAfterSave(value => !value); else if (action === "new-invoice") newInvoice(); else if (action === "submit" && !quick) void submit(); else if (action === "select-direct") setPayment(""); else if (action === "select-note") setPayment("note"); }}><KeyboardHelpButton buttonRef={keyboardHelpButton} open={keyboardHintsVisible} onToggle={() => setKeyboardHintsVisible(value => !value)} /> {stockNotice && <div className="toast stock-toast">{stockNotice}</div>}<div className="transaction-workspace pos-workspace"><div className="workspace-discovery"><ShortcutHintAnchor scope="sale" action="focus-product" visible={keyboardHintsVisible}><FramedSection title="بحث المنتجات" className="search-panel"><SearchProducts inputRef={productSearchRef} data={data} query={query} setQuery={setQuery} onPick={add} mode="sale" warehouseId={wh?.id} priceMode={priceMode} stockScope="selected-warehouse" collapseResultsWhenIdle /></FramedSection></ShortcutHintAnchor><InvoiceQuickBrowser title="سجل الفواتير" docs={data.documents.filter(d => d.kind === "sale" && d.status === "posted")} openDoc={id => { const document = data.documents.find(item => item.id === id); if (document) loadDocument(document); }} /></div>{invoice}{checkout}</div></section>;
...
function InvoiceEditorToolbar({ number: invoiceNumber, newInvoice, modes, scanner, scope, hintsVisible }: { number: string; newInvoice: () => void; modes?: ReactNode; scanner?: ReactNode; scope: InvoiceShortcutScope; hintsVisible: boolean }) {
  return <div className="invoice-editor-toolbar"><div className="invoice-toolbar-main"><ShortcutHintAnchor scope={scope} action="new-invoice" visible={hintsVisible}><button type="button" className="invoice-new-button" onClick={newInvoice}>فاتورة جديدة</button></ShortcutHintAnchor><span className="invoice-number-status">رقم <b>{invoiceNumber}</b></span></div>{modes && <div className="invoice-toolbar-modes">{modes}</div>}{scanner && <div className="invoice-toolbar-scanner">{scanner}</div>}</div>;
}
function CheckoutInvoiceActions({ printAfterSave, setPrintAfterSave, onDelete, disabled, scope, hintsVisible }: { printAfterSave: boolean; setPrintAfterSave: (value: boolean) => void; onDelete: () => void; disabled?: boolean; scope: InvoiceShortcutScope; hintsVisible: boolean }) {
  return <div className="checkout-invoice-actions"><ShortcutHintAnchor scope={scope} action="toggle-print" visible={hintsVisible}><button type="button" className="print-toggle" aria-pressed={printAfterSave} onClick={() => setPrintAfterSave(!printAfterSave)}><span>{printAfterSave && <i />}</span>طباعة</button></ShortcutHintAnchor><button type="button" className="invoice-void" disabled={disabled} onClick={onDelete}>حذف الفاتورة</button></div>;
}
...
function Purchases({ data, run, openDoc, editRequest, clearEditRequest, requestPrint }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void; editRequest: string | null; clearEditRequest: () => void; requestPrint: (id: string) => void }) {
  ...
  const [editingDocumentId, setEditingDocumentId] = useState<string | null>(null), [printAfterSave, setPrintAfterSave] = useState(false), [keyboardHintsVisible, setKeyboardHintsVisible] = useState(false);
  const baseline = useRef(""), quickSupplierButton = useRef<HTMLButtonElement>(null), productSearchRef = useRef<HTMLInputElement>(null), paymentRef = useRef<HTMLSelectElement>(null), supplierRef = useRef<HTMLButtonElement>(null), warehouseRef = useRef<HTMLButtonElement>(null), submittingRef = useRef(false), keyboardHelpButton = useRef<HTMLButtonElement>(null), editingDocument = editingDocumentId ? data.documents.find(document => document.id === editingDocumentId) ?? null : null;
  ...
  return <section className={`transaction-page${keyboardHintsVisible ? " keyboard-hints-active" : ""}`} onKeyDown={event => { if (event.defaultPrevented) return; const action = invoiceShortcutAction(event.nativeEvent, "purchase"); if (!action) { if (event.key === "Escape" && keyboardHintsVisible) setKeyboardHintsVisible(false); return; } if ((action === "select-direct" || action === "select-note") && isTypingTarget(event.target)) return; event.preventDefault(); if (action === "toggle-hints") setKeyboardHintsVisible(value => !value); else if (action === "focus-product") productSearchRef.current?.focus(); else if (action === "focus-party") supplierRef.current?.click(); else if (action === "focus-payment") (payment === "note" ? supplierRef.current : paymentRef.current)?.focus(); else if (action === "toggle-party-quick") quickSupplierButton.current?.click(); else if (action === "toggle-print") setPrintAfterSave(value => !value); else if (action === "new-invoice") newInvoice(); else if (action === "submit" && !quickSupplier && !addingWh) void submit(); else if (action === "select-direct") setPayment(""); else if (action === "select-note") setPayment("note"); else if (action === "focus-warehouse") warehouseRef.current?.click(); }}><KeyboardHelpButton buttonRef={keyboardHelpButton} open={keyboardHintsVisible} onToggle={() => setKeyboardHintsVisible(value => !value)} /><div className="transaction-workspace purchase-workspace">
    <div className="workspace-discovery"><ShortcutHintAnchor scope="purchase" action="focus-product" visible={keyboardHintsVisible}><FramedSection title="بحث المنتجات" className="search-panel"><SearchProducts inputRef={productSearchRef} data={data} query={query} setQuery={setQuery} onPick={pick} mode="purchase" warehouseId={warehouseId} collapseResultsWhenIdle /></FramedSection></ShortcutHintAnchor><InvoiceQuickBrowser title="سجل فواتير الشراء" docs={data.documents.filter(d => d.kind === "purchase" && d.status === "posted")} openDoc={id => { const document = data.documents.find(item => item.id === id); if (document) loadDocument(document); }} /></div>
    <FramedSection title="فاتورة الشراء" className="invoice-card workspace-invoice"><InvoiceEditorToolbar scope="purchase" hintsVisible={keyboardHintsVisible} number={editingDocument ? displayDocumentNumber(editingDocument) : String(data.nextDocumentSequences.purchase)} newInvoice={newInvoice} scanner={<BarcodeScanner products={activeProducts(data.products)} onScan={pick} enabled={purchaseScannerEnabled} onEnabledChange={setPurchaseScannerEnabled} />} /><div className={lines.length ? "invoice-preview has-items" : "invoice-preview"}><div className="erp-table-wrap invoice-preview-list"><table className="erp-table invoice-table" aria-label="منتجات فاتورة الشراء"><colgroup><col style={{width:"38%"}}/><col style={{width:"14%"}}/><col style={{width:"17%"}}/><col style={{width:"19%"}}/><col style={{width:"12%"}}/></colgroup><thead><tr><th>الاسم</th><th>الكمية</th><th>سعر الشراء</th><th>المجموع</th><th>حذف</th></tr></thead><tbody>{details.map(({line, product}) => <tr key={product.id} onClick={() => setSelectedLine(product.id)} className={selectedLine === product.id ? "selected" : ""}><td className="name-cell">{product.name}</td><td className="num-cell"><Num value={line.quantity} onChange={value => updatePurchaseLine(product, { quantity: value })} /></td><td className="num-cell"><Num value={line.unitPrice} onChange={value => updatePurchaseLine(product, { unitPrice: value })} /></td><td className="num-cell">{number(val(line.quantity) * val(line.unitPrice))}</td><td className="action-cell"><button type="button" className="row-delete" aria-label={`حذف ${product.name}`} onClick={event => { event.stopPropagation(); setLines(current => current.filter(item => item.productId !== product.id)); }}><X /></button></td></tr>)}{!details.length && <tr className="invoice-empty-row"><td colSpan={5}>الفاتورة فارغة</td></tr>}</tbody></table></div></div></FramedSection>
    <FramedSection title="الدفع" className="workspace-checkout purchase-checkout"><div className="checkout-layout"><div className="checkout-body purchase-details"><div className="invoice-meta-row"><ShortcutHintAnchor scope="purchase" action="select-direct" visible={keyboardHintsVisible}><button className="meta-option selection-option" aria-pressed={payment !== "note"} onClick={() => setPayment("")}><Banknote /><span><small>نوع التسوية</small><b>دفع مباشر</b></span></button></ShortcutHintAnchor><ShortcutHintAnchor scope="purchase" action="select-note" visible={keyboardHintsVisible}><button className="meta-option selection-option secondary" aria-pressed={payment === "note"} onClick={() => setPayment("note")}><PencilLine /><span><small>نوع التسوية</small><b>ملاحظة</b></span></button></ShortcutHintAnchor></div><span className="payment-label">{payment === "note" ? "المورد" : "طريقة الدفع"}</span><div className={`purchase-payment-row${payment === "note" ? " note" : ""}`}>{payment !== "note" && <ShortcutHintAnchor scope="purchase" action="focus-payment" visible={keyboardHintsVisible}><CompactPaymentSelector selectRef={paymentRef} accounts={data.paymentAccounts} value={payment} onChange={setPayment} /></ShortcutHintAnchor>}<ShortcutHintAnchor scope="purchase" action="focus-party" visible={keyboardHintsVisible} className="purchase-supplier"><SearchableSelect triggerRef={supplierRef} value={partyId} onChange={setPartyId} placeholder="اختر المورد" searchPlaceholder="ابحث باسم المورد أو رقم الهاتف" floating options={data.parties.filter(p => resolvePartyType(p) === "supplier").map(p => ({ value: p.id, label: p.name, search: p.phone }))} /></ShortcutHintAnchor><ShortcutHintAnchor scope="purchase" action="toggle-party-quick" visible={keyboardHintsVisible}><button ref={quickSupplierButton} type="button" className="pos-quick-customer-button" aria-label="إضافة المورد" onClick={() => setQuickSupplier(value => !value)}><Plus /><span>إضافة المورد</span></button></ShortcutHintAnchor></div>{quickSupplier && <QuickSupplier anchor={quickSupplierButton} run={run} cancel={() => setQuickSupplier(false)} onDone={id => { setPartyId(id); setQuickSupplier(false); }} />}<CheckoutInvoiceActions scope="purchase" hintsVisible={keyboardHintsVisible} printAfterSave={printAfterSave} setPrintAfterSave={setPrintAfterSave} onDelete={deletePurchase} disabled={!editingDocument && !lines.length} /><ShortcutHintAnchor scope="purchase" action="focus-warehouse" visible={keyboardHintsVisible}><label>مخزن الاستلام<SearchableSelect triggerRef={warehouseRef} value={warehouseId} onChange={setWarehouseId} placeholder="اختر المخزن" searchPlaceholder="ابحث عن مخزن" floating options={activeWarehouses(data.warehouses).map(w => ({ value: w.id, label: w.name }))} /></label></ShortcutHintAnchor><button className="link purchase-add-warehouse" onClick={() => setAddingWh(!addingWh)}><Plus /> إضافة مخزن</button>{addingWh && <InlineCreate label="اسم المخزن" onSave={async name => { await run({ type: "warehouse.create", name }, "تمت إضافة المخزن"); setAddingWh(false); }} />}</div><div className="checkout-footer"><div className="total invoice-total"><span>الإجمالي</span><strong>{money(total)}</strong></div><ShortcutHintAnchor scope="purchase" action="submit" visible={keyboardHintsVisible}><button title="إنهاء الشراء (F9)" className="primary wide" disabled={!partyId || !warehouseId || !lines.length || (payment !== "note" && !payment)} onClick={() => void submit()}>{editingDocumentId ? "حفظ التعديلات" : "إتمام الشراء"}</button></ShortcutHintAnchor></div></div></FramedSection>
  </div></section>;
