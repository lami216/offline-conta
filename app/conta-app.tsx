...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const openOverviewSummary=(key:"currentAccountsBalance"|"currentInventoryValue"|"currentReceivable"|"currentPayable"|"sales"|"purchases"|"expenses"|"netOperatingResult")=>{if(!result)return;const details=result.overviewDetails;
    ...
    if(key==="sales")return setSummaryDetail({title:tr("صافي المبيعات"),total:reportNumber(result.summary.sales),rows:overviewDocumentRows(details?.sales,row=>row.kind==="return"?tr("حركة تاريخية"):tr("فاتورة بيع"))});
    if(key==="purchases")return setSummaryDetail({title:tr("إجمالي المشتريات"),total:reportNumber(result.summary.purchases),rows:overviewDocumentRows(details?.purchases,()=>tr("فاتورة شراء"))});
    if(key==="expenses")return setSummaryDetail({title:tr("إجمالي المصاريف"),total:reportNumber(result.summary.expenses),rows:overviewDocumentRows(details?.expenses,()=>tr("فاتورة مصروفات"))});
