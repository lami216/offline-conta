...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const movementColumns=useMemo(()=>[{key:"date",type:"date" as const,get:(m:BootstrapData["financialMovements"][number])=>m.occurredAt},{key:"type",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>movementLabels[financialMovementKind(m.type)]??m.type},{key:"account",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>name(m.paymentMethod)},{key:"amount",type:"money" as const,get:(m:BootstrapData["financialMovements"][number])=>m.direction==="in"?m.amount:-m.amount},{key:"document",type:"text" as const,get:(m:BootstrapData["financialMovements"][number])=>m.documentNumber}], [data.paymentAccounts,movementLabels,name]),{sort:movementSort,sortedRows:sortedMovements,toggle:toggleMovementSort}=useSortableRows(movements,movementColumns);
  const transferColumns=useMemo(()=>[{key:"date",type:"date" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.occurredAt},{key:"from",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>name(t.fromAccountId)},{key:"to",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>name(t.toAccountId)},{key:"amount",type:"money" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.amount},{key:"reference",type:"text" as const,get:(t:BootstrapData["accountTransfers"][number])=>t.number}], [data.paymentAccounts,name]),{sort:transferSort,sortedRows:sortedTransfers,toggle:toggleTransferSort}=useSortableRows(transfers,transferColumns);
