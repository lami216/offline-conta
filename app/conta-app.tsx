    ...
    <div className={`app-shell section-${view}`} dir={dir}>
      ...
      <main>
        ...
        <header className="page-bar">
          ...
          <button className="icon refresh" title={tr("تحديث البيانات")} aria-label={tr("تحديث البيانات")} onClick={requestPageRefresh}><RefreshCw /></button><div className="page-utilities"><time dateTime={today.toISOString().slice(0,10)}>{localizedDate(today,{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</time><button className="language-switch soft" type="button" onClick={()=>setLocale(locale==="ar"?"fr":"ar")}><Globe/>{locale==="ar"?"Français":"العربية"}</button></div>
        </header>
