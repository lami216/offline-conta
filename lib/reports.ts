...
export async function buildReport(db: Db, f: ReportFilters): Promise<ReportResponse> {
  ...
  if (f.type === "stock" || f.type === "financial") {
    ...
    rows=raw.map(x=>({id:String(x.id),documentId:String(x.documentId),occurredAt:String(x.occurredAt),paymentMethod:String(x.paymentMethod),movementType:financialMovementKind(x.type),incoming:x.direction==="in"?n(x.amount):0,outgoing:x.direction==="out"?n(x.amount):0,party:String(x.partyName??""),documentNumber:String(x.documentNumber??"")}));const operating=all.filter(x=>isOperatingFinancialMovement(x.type)),businessIncoming=operating.filter(x=>x.direction==="in").reduce((s,x)=>s+n(x.amount),0),businessOutgoing=operating.filter(x=>x.direction==="out").reduce((s,x)=>s+n(x.amount),0),balanceNet=all.reduce((s,x)=>s+(x.direction==="in"?n(x.amount):-n(x.amount)),0);return{report:f.type,from:f.from!,to:f.to!,summary:{incoming:all.filter(x=>x.direction==="in").reduce((s,x)=>s+n(x.amount),0),outgoing:all.filter(x=>x.direction==="out").reduce((s,x)=>s+n(x.amount),0),net:balanceNet,businessIncoming,businessOutgoing,businessNet:businessIncoming-businessOutgoing,balanceNet,operatingIncoming:businessIncoming,operatingOutgoing:businessOutgoing,operatingNet:businessIncoming-businessOutgoing},rows,meta:pagination(totalRows,f)};
  }
