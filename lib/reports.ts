...
export async function buildReport(db: Db, f: ReportFilters): Promise<ReportResponse> {
  ...
  if (f.type === "sales") {
    ...
    const summaryFacts = await saleFacts(db, [...sales.all, ...legacySaleAdjustments], f, categoryScope), totals = profitSummary(summaryFacts),pageIds=new Set(sales.rows.map(document=>String(document.id))),pageFacts=summaryFacts.filter(fact=>pageIds.has(String(fact.documentId)));
    const rows = hasProductFilter ? pageFacts : sales.rows.map(document => { const p = profitSummary(pageFacts.filter(fact=>String(fact.documentId)===String(document.id))); return { id: String(document.id), documentId: String(document.id), number: displayDocumentNumber(document), occurredAt: String(document.occurredAt), party: String(document.partyName ?? "بيع مباشر"), paymentMethod: String(document.paymentMethod ?? ""), total: n(document.total), cost: p.cost, profit: p.profit, margin: n(document.total) ? p.profit / n(document.total) * 100 : 0, paid: n(document.paidTotal), due: n(document.dueTotal) }; });
    return { report: f.type, from: f.from!, to: f.to!, summary: { count: sales.totalRows, grossSales: sales.all.reduce((s, d) => s + (hasProductFilter ? (d.lines as Document[]).filter(l => lineMatches(l, f, categoryScope)).reduce((x,l)=>x+n(l.lineTotal),0) : n(d.total)), 0), netSales: totals.revenue, cost: totals.cost, profit: totals.profit, margin: totals.margin, unknownRevenue: totals.unknownRevenue, paid: sales.all.reduce((s,d)=>s+n(d.paidTotal),0), due: sales.all.reduce((s,d)=>s+n(d.dueTotal),0) }, rows, meta: pagination(sales.totalRows, f) };
  }
  if (f.type === "purchases" || f.type === "expenses") {
    ...
    const rows: ReportRow[] = found.rows.flatMap<ReportRow>(document => { const selected = ((document.lines ?? []) as Document[]).filter(line => lineMatches(line, f, categoryScope)),party=String(partyNames.get(String(document.partyId))??document.partyName??""); if (f.type === "purchases" && hasProductFilter) return selected.map(line => ({ id:`${document.id}-${line.id ?? line.productId}`,documentId:String(document.id),number:displayDocumentNumber(document),occurredAt:String(document.occurredAt),party,product:String(identities.get(String(line.productId))?.name??line.description??"").trim()||"منتج غير متاح",sku:String(identities.get(String(line.productId))?.sku??line.sku??"—")||"—",quantity:n(line.quantity),unitPrice:n(line.unitPrice),total:n(line.lineTotal) })); return [{ id:String(document.id),documentId:String(document.id),number:displayDocumentNumber(document),occurredAt:String(document.occurredAt),party,paymentMethod:String(document.paymentMethod??""),title:String(document.title??""),recurring:Boolean(document.recurringId),total:n(document.total),paid:n(document.paidTotal),due:n(document.dueTotal) }]; });
    const value = (d: Document) => hasProductFilter ? (d.lines as Document[]).filter(l=>lineMatches(l,f,categoryScope)).reduce((s,l)=>s+n(l.lineTotal),0) : n(d.total);
    return { report:f.type,from:f.from!,to:f.to!,summary:{count:found.totalRows,total:found.all.reduce((s,d)=>s+value(d),0),quantity:found.all.reduce((s,d)=>s+(d.lines as Document[]).filter(l=>lineMatches(l,f,categoryScope)).reduce((x,l)=>x+n(l.quantity),0),0),paid:found.all.reduce((s,d)=>s+n(d.paidTotal),0),due:found.all.reduce((s,d)=>s+n(d.dueTotal),0),recurringTotal:found.all.filter(d=>Boolean(d.recurringId)).reduce((s,d)=>s+n(d.total),0),oneOffTotal:found.all.filter(d=>!d.recurringId).reduce((s,d)=>s+n(d.total),0)},rows,meta:pagination(found.totalRows,f) };
  }
  if (f.type === "stock") {
  const query: Document = matchDate(f);
  if (constraint) query.productId = constraint;
  const stored = await db.collection("stockMovements").find(query).toArray() as Array<Record<string, unknown>>;
  const documentIds = [...new Set(stored.map(row => String(row.documentId ?? "")).filter(Boolean))];
  const documents = (documentIds.length ? await db.collection("documents").find({ id: { $in: documentIds } }).project({ id: 1, number: 1, kind: 1, title: 1, openingCorrection: 1, openingStockBefore: 1, openingStockAfter: 1 }).toArray() : []) as Array<Record<string, unknown>>;
  const documentMap = new Map(documents.map(document => [String(document.id), document]));
  const classified: Array<Record<string, unknown> & { type: string }> = stored.map(row => ({ ...row, type: classifyStockMovementType(row.type, documentMap.get(String(row.documentId ?? ""))) }));
  const all = f.movementType ? classified.filter(row => stockMovementMatchesFilter(row.type, f.movementType)) : classified;
  const ordered = [...all].sort((a, b) => String(b.occurredAt).localeCompare(String(a.occurredAt)));
  const totalRows = all.length, pageStart = ((f.page ?? 1) - 1) * (f.pageSize ?? 100), raw = f.unpaged ? ordered : ordered.slice(pageStart, pageStart + (f.pageSize ?? 100));
  const products = await db.collection("products").find({ id: { $in: raw.map(row => row.productId) } }).project({ id: 1, sku: 1, name: 1 }).toArray(), identities = new Map(products.map(product => [String(product.id), product]));
  const rows: ReportRow[] = raw.map(row => ({ id: String(row.id), documentId: String(row.documentId), occurredAt: String(row.occurredAt), sku: String(identities.get(String(row.productId))?.sku ?? row.sku ?? "—") || "—", product: String(identities.get(String(row.productId))?.name ?? row.productName ?? "").trim() || "منتج غير متاح", warehouse: String(row.warehouseName), movementType: String(row.type), before: n(row.balanceBefore), change: n(row.quantityDelta), after: n(row.balanceAfter), documentNumber: String(row.documentNumber) }));
...
if (f.type === "financial") {
  const query: Document = { ...matchDate(f), status: { $ne: "reversed" }, isReversal: { $ne: true } };
  if (f.movementType) query.type = ["sale", "purchase"].includes(f.movementType) ? { $regex: `^${f.movementType}(?::|$)` } : f.movementType;
  if (f.paymentAccountId) query.paymentMethod = f.paymentAccountId;
  if (f.direction) query.direction = f.direction;
  const all = await db.collection("financialMovements").find(query).toArray(), totalRows = all.length, ordered = [...all].sort((a, b) => String(b.occurredAt).localeCompare(String(a.occurredAt))), raw = f.unpaged ? ordered : ordered.slice((f.page - 1) * f.pageSize, f.page * f.pageSize);
  const rows = raw.map(row => ({ id: String(row.id), documentId: String(row.documentId), occurredAt: String(row.occurredAt), paymentMethod: String(row.paymentMethod), movementType: financialMovementKind(row.type), incoming: row.direction === "in" ? n(row.amount) : 0, outgoing: row.direction === "out" ? n(row.amount) : 0, party: String(row.partyName ?? ""), documentNumber: String(row.documentNumber ?? "") }));
  ...
  if (f.type === "debts") { const query:Document={isArchived:{$ne:true}};if(f.search){const literal=escapeRegex(f.search);query.$or=[{name:{$regex:literal,$options:"i"}},{phone:{$regex:literal}}]};if(f.debtSide==="receivable")query.receivable={$gt:0};if(f.debtSide==="payable")query.payable={$gt:0};if(f.debtSide==="clear")query.$and=[{receivable:{$lte:0}},{payable:{$lte:0}}];const all=await db.collection("parties").find(query).toArray(),total=all.length,ordered=[...all].sort((a,b)=>{const x=String(a.name??""),y=String(b.name??"");return x===y?0:x<y?-1:1}),raw=f.unpaged?ordered:ordered.slice((f.page-1)*f.pageSize,f.page*f.pageSize),rows=raw.map(p=>({id:String(p.id),partyId:String(p.id),name:String(p.name),phone:String(p.phone??""),partyType:String(p.partyType),accountType:p.partyType==="customer"?"عميل":"مورد",balance:Math.abs(n(p.receivable)-n(p.payable)),receivable:Math.max(n(p.receivable)-n(p.payable),0),payable:Math.max(n(p.payable)-n(p.receivable),0),lastMovement:String(p.lastMovementAt??"")}));return{report:f.type,from:null,to:null,summary:{receivable:all.reduce((s,p)=>s+Math.max(n(p.receivable)-n(p.payable),0),0),payable:all.reduce((s,p)=>s+Math.max(n(p.payable)-n(p.receivable),0),0),net:all.reduce((s,p)=>s+n(p.receivable)-n(p.payable),0),count:total},rows,meta:pagination(total,f)}; }
  if (f.type === "party-ledger") {
    ...
    const all = await db.collection("documents").find(query).toArray(), total = all.length;
    const ordered = [...all].sort((a,b)=>String(b.occurredAt).localeCompare(String(a.occurredAt)));
    const raw = f.unpaged ? ordered : ordered.slice((f.page - 1) * f.pageSize, f.page * f.pageSize);
