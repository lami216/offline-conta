...
function ensureAtLeastOneLoginUser(db:Database.Database){const exists=db.prepare("SELECT 1 FROM users WHERE json_extract(data_json,'$.isActive')=1 LIMIT 1").get();if(exists)return;const now=new Date().toISOString(),owner={id:"owner",username:"المالك",usernameNormalized:"المالك",name:"المالك",passwordHash:hashPassword("12345678"),isActive:true,owner:true,createdAt:now,updatedAt:now,_id:"owner"};db.prepare("INSERT INTO users(record_key,data_json) VALUES('owner',?) ON CONFLICT(record_key) DO UPDATE SET data_json=excluded.data_json").run(encode(owner))}
