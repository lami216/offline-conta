...
function ensureOwnerSeed(db:Database.Database){const exists=db.prepare("SELECT 1 FROM users WHERE record_key='owner'").get();if(exists)return;const now=new Date().toISOString(),owner={id:"owner",username:"المالك",usernameNormalized:"المالك",name:"المالك",passwordHash:hashPassword("12345678"),isActive:true,owner:true,createdAt:now,_id:"owner"};db.prepare("INSERT INTO users(record_key,data_json) VALUES('owner',?)").run(encode(owner))}
