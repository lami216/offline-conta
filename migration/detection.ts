...
export async function detectImportFile(bytes:Uint8Array):Promise<DetectionResult>{
 try{parseAndValidateBackup(new TextDecoder().decode(bytes));return{kind:"conta-backup",sourceType:"conta-backup",label:"نسخة الكرنة",needsManualMapping:false}}catch{}
