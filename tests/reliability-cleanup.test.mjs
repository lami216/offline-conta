...
const app=await readFile(new URL("../app/conta-app.tsx",import.meta.url),"utf8"),route=await readFile(new URL("../app/api/command/route.ts",import.meta.url),"utf8"),mongo=await readFile(new URL("../lib/mongodb.ts",import.meta.url),"utf8");
...
test("server receipt claim and business mutation share a transaction",()=>{assert.match(route,/receipts\.insertOne\([^;]+\{session\}/);assert.match(route,/result=await execute\(db,session,body\)/);assert.match(route,/status:"committed",result:response/);assert.match(route,/مفتاح العملية مستخدم لطلب مختلف/);assert.match(route,/العملية قيد التنفيذ/);assert.match(mongo,/commandReceipts[^\n]+expireAfterSeconds: 7 \* 24 \* 60 \* 60/)});
