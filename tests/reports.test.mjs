...
test("legacy sale adjustments preserve historical profit independently of current product cost",async()=>{await db.dropDatabase();await db.collection("products").insertOne({id:"a",lastPurchaseCost:999});await db.collection("documents").insertMany([doc("sale","sale","2026-08-10",[line("s","a",10,100,70)]),doc("ret","return","2026-08-11",[line("r","a",2,100,70)],{parentDocumentId:"sale"})]);const report=await buildReport(db,filters("profit"));assert.equal(report.summary.revenue,800);assert.equal(report.summary.cost,560);assert.equal(report.summary.profit,240);});
