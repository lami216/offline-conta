...
test("report periods and strict paging/group/sort allowlists",()=>{const value=parse("type=sales&from=2026-08-01&to=2026-08-21&page=2&pageSize=100&sortKey=total&sortDirection=desc");assert.deepEqual([value.from,value.to,value.page,value.pageSize,value.sortKey,value.sortDirection],["2026-08-01","2026-08-21",2,100,"total","desc"]);assert.throws(()=>parse("type=sales&from=2026-08-22&to=2026-08-21"),/بداية الفترة/);assert.throws(()=>parse("type=tax&from=2026-08-01&to=2026-08-21"),/نوع التقرير/);assert.throws(()=>parse("type=profit&from=2026-08-01&to=2026-08-21&groupBy=party"),/groupBy/);assert.throws(()=>parse("type=sales&from=2026-08-01&to=2026-08-21&sortKey=unknown"),/ترتيب التقرير/);});
...
test("paged report sorting is global while summaries remain complete",async()=>{await db.collection("documents").insertMany([50,10,40,20,30].map((total,index)=>doc(`sort-${index}`,"sale",`2026-08-${String(10+index).padStart(2,"0")}`,[line(`l-${index}`,"a",1,total,total/2)])));const first=await buildReport(db,filters("sales",{page:1,pageSize:2,sortKey:"total",sortDirection:"desc"})),second=await buildReport(db,filters("sales",{page:2,pageSize:2,sortKey:"total",sortDirection:"desc"}));assert.deepEqual(first.rows.map(row=>row.total),[50,40]);assert.deepEqual(second.rows.map(row=>row.total),[30,20]);assert.deepEqual([first.meta.totalRows,first.meta.totalPages,first.summary.netSales],[5,3,150]);const full=await buildReport(db,filters("sales",{page:1,pageSize:1,sortKey:"total",sortDirection:"asc",unpaged:true}));assert.deepEqual(full.rows.map(row=>row.total),[10,20,30,40,50]);assert.deepEqual([full.meta.page,full.meta.totalPages],[1,1])});
test("product-filtered purchase paging counts visible line rows without changing invoice totals",async()=>{await db.collection("products").insertOne({id:"a",name:"A",sku:"A"});await db.collection("documents").insertMany([doc("p1","purchase","2026-08-10",[line("1","a",1,10),line("2","a",2,11)]),doc("p2","purchase","2026-08-11",[line("3","a",3,12)])]);const report=await buildReport(db,filters("purchases",{productId:"a",pageSize:2}));assert.deepEqual([report.meta.totalRows,report.meta.totalPages,report.rows.length,report.summary.count,report.summary.total],[3,2,2,2,68])});
