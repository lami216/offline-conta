...
test('zero users enables direct local access, while creating the first user starts authenticated access',async()=>{
 const local=await auth.getPrincipalFromRequest(apiRequest('/','GET'));assert.equal(local.principalType,'local');assert.ok(local.permissions.includes('settings.users.manage'));
