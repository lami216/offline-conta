...
test("stock operations collapse idle search and edit a serial ERP draft", () => {
  ...
  assert.match(css, /\.stock-workspace\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*64fr\)\s+minmax\(410px,\s*36fr\)/s);
...
test("adjustment workspace explicitly places all three desktop panels", () => {
  ...
  assert.match(css, /\.adjustment-workspace\s*>\s*\.opening-stock-history\s*\{[^}]*grid-column:\s*2[^}]*grid-row:\s*2/s);
});
